...
CktPiCell::updateHierConn(string& prefix, InPinMap& inPinMap) const //for non-topNodule
{
   ...
   //QuteMsg(MSG_IFO) << "PI hierName = " << hierName << endl;
   ...
   for (unsigned i = 0; i < getOutPin()->getInPinSize(); ++i) {
      ...
      //QuteMsg(MSG_IFO) << "foutCell type = " << foutCell->getCellType() << endl;
      if (foutCell->getCellType() == CKT_MODULE_CELL)  //PI --> M
         ;//handle in CkeModuleCell
      else if (foutCell->getCellType() == CKT_PO_CELL)  // PI --> PO
         QuteMsg(MSG_ERR) << "error : Should have buf between PI and PO @CktPiCell::updateHierConn()" << endl;
      else {  //PI --> C
         //QuteMsg(MSG_IFO) << "======================CktPiCell=======================" << endl;
         //QuteMsg(MSG_IFO) << "PI Name = " << getOutPin()->getName() << endl;
         //QuteMsg(MSG_IFO) << "FO Name = " << getOutPin()->getInPin(i)->getOutPin()->getName() << endl;
         //QuteMsg(MSG_IFO) << "FO type = " << foutCell->getCellType() << endl;
...
CktPiCell::writeBLIF_PI() const
{
   ...
   //QuteMsg(MSG_IFO) << "CktPiCell InPinSize = " << getInPinSize() << endl;
...
/* // jojoman have change the connect, so never have the connection "PO->C"
   ...
   for (int j = 0; j < getOutPin()->getInPinSize(); ++j) {
      ...
      if (foutCellType == CKT_MODULE_CELL)
         ...
         QuteMsg(MSG_ERR) << "warning : XXXXXXXXXXXXXXXXXX" << endl;
         / *if (isTopModule) {
         ...
         QuteMsg(MSG_ERR) << "Error : CKT_PI_CELL or CKT_PIO_CELL @CktModuleCell::updateHierConn(...)" << endl;
      else {
         // known bug in syn
...
{
   QuteMsg(MSG_ERR) << "Still not finish the function \"CktPioCell::updateHierConn\"" << endl;
   exit(0);
}
