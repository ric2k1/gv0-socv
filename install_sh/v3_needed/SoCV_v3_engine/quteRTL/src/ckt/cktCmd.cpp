...
CreateDesignCmd::exec(const string& option) {
   ...
   QuteMsg(MSG_IFO) << "New Design \"" << curCktModule->getModuleName() << " has been created !!!" << endl;
   return CMD_EXEC_DONE;
}

void
CreateDesignCmd::usage() const {
   QuteMsg(MSG_IFO) << "Usage: CREate DEsign <topModuleName>" << endl;
}

void
CreateDesignCmd::help() const {
   QuteMsg(MSG_IFO) << setw(20) << left << "CREate DEsign:" << "Create design from command line interface." << endl;
}

bool
CreateDesignCmd::valid() const {
   if (VLDesign.isDesignRead())
      QuteMsg(MSG_WAR) << "Design exists currently !!!" << endl;
   else if (VLDesign.isFlatModule())
      QuteMsg(MSG_WAR) << "Design has already been flattened !!!" << endl;
   else return true;
   return false;
}
...
DefineNetCmd::exec(const string& option) {
   ...
   if (pi) {
      if (curCktModule->createPort(CKT_IN, netName, width - 1, 0))
         QuteMsg(MSG_DBG) << "Primary Input \"" << netName << "[" << width - 1 << " : " << 0 << "]\" has been created !!!" << endl;
   }
   else if (po) {
      if (curCktModule->createPort(CKT_OUT, netName, width - 1, 0))
         QuteMsg(MSG_DBG) << "Primary Output \"" << netName << "[" << width - 1 << " : " << 0 << "]\" has been created !!!" << endl;
   }
   else if (pio) {
      if (curCktModule->createPort(CKT_IO, netName, width - 1, 0))
         QuteMsg(MSG_DBG) << "Primary Inout \"" << netName << "[" << width - 1 << " : " << 0 << "]\" has been created !!!" << endl;
   }
   else {
      if (curCktModule->createNet(netName, width - 1, 0))
         QuteMsg(MSG_DBG) << "Net \"" << netName << "[" << width - 1 << " : " << 0 << "]\" Created !!!" << endl;
   }
...
DefineNetCmd::usage() const {
   QuteMsg(MSG_IFO) << "Usage: DEFine NET [-PI | -PO | -PIO] [(unsigned width)] <(string NetName)>" << endl;
}

void
DefineNetCmd::help() const {
   QuteMsg(MSG_IFO) << setw(20) << left << "DEFine NET:" << "Create I/O ports and nets on current design." << endl;
}

bool
DefineNetCmd::valid() const {
   if (!VLDesign.isDesignRead())
      QuteMsg(MSG_WAR) << "Design does not exist !!!" << endl;
   else if (!VLDesign.isHierModule())
      QuteMsg(MSG_WAR) << "Design has not been synthesized !!!" << endl;
   else if (VLDesign.isFlatModule())
      QuteMsg(MSG_WAR) << "Design has already been flattened !!!" << endl;
   else return true;
   return false;
...
DefineCellCmd::exec(const string& option) {
   ...
   switch (cellType) {
      ...
      case CKT_MERGE_CELL : 
         ...
         break;
      default : 
         QuteMsg(MSG_WAR) << "Cell Type \"" << options[0] << "\" has not implemented ?!?!?!" << endl;
         break;
   }
   if (result) QuteMsg(MSG_DBG) << options[0] << " CELL \"" << options[1] << "\" has been created !!!" << endl;
...
DefineCellCmd::usage() const {
   QuteMsg(MSG_IFO) << "Usage: DEFine CELL <CellType> <Output_NetName> <Input_NetName1> ..." << endl;
   QuteMsg(MSG_IFO) << "       Supports : 1  Input Cells : BUF, INV, AND, OR, NAND, NOR, XOR, XNOR, CONST" << endl;
   QuteMsg(MSG_IFO) << "                  2  Input Cells : AND, OR, NAND, NOR, XOR, XNOR, ADD, SUB, MULT, DIV, MOD, SHL, SHR, EQ, GEQ, GT, LEQ, LESS" << endl;
   QuteMsg(MSG_IFO) << "                  3  Input Cells : MUX, SLICE" << endl;
   QuteMsg(MSG_IFO) << "                  2+ Input Cells : MERGE" << endl;
   QuteMsg(MSG_IFO) << "                  Sequential     : DFF <Output_NetName> <Input_Name> <Input_ClockName> <Input_ResetName> [Input_DefaultName]" << endl;
   QuteMsg(MSG_IFO) << "                                   DLAT <Output_NetName> <Input_Name> [Input_LoadName]" << endl;
}

void
DefineCellCmd::help() const {
   QuteMsg(MSG_IFO) << setw(20) << left << "DEFine CEll:" << "Create cell(s) with specified cell type on current design." << endl;
}
...
DefineInstCmd::exec(const string& option) {
   ...
   if (curCktModule->createModuleCell(options[0], options[1], ports))
      QuteMsg(MSG_DBG) << "Module Inst \"" << options[1] << "\" of Module \"" << options[0] << "\" has been created !!!" << endl;
...
DefineInstCmd::usage() const {
   QuteMsg(MSG_IFO) << "Usage: DEFine INST <(string moduleName)> <(string instName)> <port_Name> ..." << endl;
}

void
DefineInstCmd::help() const {
   QuteMsg(MSG_IFO) << setw(20) << left << "DEFine INST:" << "Create module instance." << endl;
}
...
DefineModuleCmd::exec(const string& option) {
   ...
   if (!module) QuteMsg(MSG_ERR) << "Create New Module Failed !!!";
   else {
      curCktModule = module;
      QuteMsg(MSG_IFO) << "New Module \"" << curCktModule->getModuleName() << " has been created !!!" << endl;
   }
   return CMD_EXEC_DONE;
}
...
DefineModuleCmd::usage() const {
   QuteMsg(MSG_IFO) << "Usage: DEFine MODule <(string modulename)>" << endl;
}

void
DefineModuleCmd::help() const {
   QuteMsg(MSG_IFO) << setw(20) << left << "DEFine MODule:" << "Create a new sub-module on current design." << endl;
}
...
// check option 
   ...
   if (!module) QuteMsg(MSG_ERR) << "NO Upper Module in the Hierarchy" << endl;
   else {
      curCktModule = module;
      QuteMsg(MSG_IFO) << "Back to Module \"" << curCktModule->getModuleName() << " !!!" << endl;
   }
...
ChangeModuleCmd::usage() const {
   QuteMsg(MSG_IFO) << "Usage: CHAnge MODule" << endl;
}

void
ChangeModuleCmd::help() const {
   QuteMsg(MSG_IFO) << setw(20) << left << "CHAnge MODule:" << "Change current module to its parant module in current design hierarchy." << endl;
}
...
WriteCfgCmd::exec(const string& option) {
   ...
   if (VLDesign.getCktModule((VLDesign.getTopModName()))->writeCDFG(pinName, fileName, bits)) {
      ...
      QuteMsg(MSG_IFO) << "Write file " << fileName << " complete!";
   }
...
WriteCfgCmd::usage() const {
   QuteMsg(MSG_IFO) << "Usage: WRIte CFG <(string pinName)> <(string fileName)> [| -DOT | -PNG | -PS]" << endl;
   QuteMsg(MSG_IFO) << "                 [| -All | -Bit <(unsigned bit1)> <(unsigned bit2)> ...]" << endl;
}

void
WriteCfgCmd::help() const {
   QuteMsg(MSG_IFO) << setw(20) << left << "WRIte CFG:" << "Write out the control-flow graph for a specific pin with assigned bits." << endl;
}
...
WriteCfgCmd::valid() const {
   if (!VLDesign.isDesignRead())
      ...
      QuteMsg(MSG_WAR) << "Input design was not parsed into CDFG !!!" << endl;
   else return true;
   return false;
}
...
WriteDfgCmd::exec(const string& option) 
{
   ...
   if (!OutPin) { QuteMsg(MSG_ERR) << "Assigned gateName NOT exists!" << endl; return CMD_EXEC_DONE; }
...
{
   QuteMsg(MSG_IFO) << "Usage: WRIte DFG <(string gateName)> <(string fileName)> [| -DOT | -PNG | -PS] " << endl;
   QuteMsg(MSG_IFO) << "                 < < -Level <(unsigned nLevel)> > | < -Depth <(unsigned nDepth) [| -Time_flow | -Bound_reg] > >" << endl;
   QuteMsg(MSG_IFO) << "       -Level     : # propagated gates level" << endl;
   QuteMsg(MSG_IFO) << "       -Depth     : # past clock cycles" << endl;
   QuteMsg(MSG_IFO) << "       -Time_flow : In output, INs are PIs and FFs, OUT is the gate assigned" << endl;
   QuteMsg(MSG_IFO) << "       -Bound_reg : In output, INs are PIs and FFs, OUTs are the gate assigned and FFs" << endl;
}

void
WriteDfgCmd::help() const 
{
   QuteMsg(MSG_IFO) << setw(20) << left << "WRIte DFG:" << "Write out the data-flow graph of assigned gate in flattened design." << endl;
}
...
WriteDfgCmd::valid() const
{
   if (!VLDesign.isDesignRead())
      ...
      QuteMsg(MSG_WAR) << "Design has not been flattened !!!" << endl;
   else return true;
   return false;
}
...
WhoAmICmd::exec(const string& option) 
{
   ...
   else QuteMsg(MSG_IFO) << OutPin->getCell()->getExpr(OutPin);
   return CMD_EXEC_DONE;
}
...
{
   QuteMsg(MSG_IFO) << "Usage: WhoAmI <(string signalName)>" << endl;
}

void
WhoAmICmd::help() const 
{
   QuteMsg(MSG_IFO) << setw(20) << left << "WHOAMI:" << "Get Expression of signal in terms of declared ones." << endl;
}
