...
{
   stringstream ss;
   QuteMsg(MSG_ERR) << "genWireName!!!!!!!!" << end;
   ...
   ++netNumber;
   //QuteMsg(MSG_IFO)<< "netName: "<< ss.str()<< endl;
   return ss.str();
}*/
...
CktCell::genOutput() const
{
   if (getInPinSize() == 1)
      ...
      QuteMsg(MSG_WAR) << "Cell " << getOutPin()->getName() << " has InPinSize = " << getInPinSize() << " !" << endl;
      //assert (0);
      //QuteMsg(MSG_ERR) << "Warning : @ CktCell::genOutput()" << endl;
   }
}
