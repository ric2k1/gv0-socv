...
CktBusCell::writeBLIF() const
{
   ...
   //QuteMsg(MSG_IFO) << "no finish in CktBusCell::writeBLIF()" << endl;
   //for (int i = 0; i < getInPinSize(); ++i)
   //   QuteMsg(MSG_IFO) << i << " : " << getFinCell(i)->getCellType() << endl;   
   stringstream outFile;
   ...
   if (isReverse == false) // check the outpin isReverse
      QuteMsg(MSG_ERR) << "Warning : Bits reverse! Need to handle!! It doesn't finish(@CktBusCell::writeBLIF())" << endl;
//pre-process
   for (unsigned i = 0; i < getInPinSize(); ++i)
   {
      //known bug
...
CktBusCell::checkIoWidth() const
{
   ...
   QuteMsg(MSG_IFO) << "Bus Cell name = " << getOutPin()->getName();
   QuteMsg(MSG_IFO) << " begin = " << bus->getBegin() << " end = " << bus->getEnd() << endl;
   QuteMsg(MSG_IFO) << "===fanin Cell===" << endl;
   for (unsigned i = 0; i < getInPinSize(); ++i) {
      outPin = getInPin(i)->getOutPin();
      bus = VLDesign.getBus(outPin->getBusId());
      QuteMsg(MSG_IFO) << "   No " << i 
...
CktBusCell::writeBLIF() const
{
   ...
   QuteMsg(MSG_IFO) << "1. begin, end = " << begin << ", " << end << endl;
   ...
   if (isReverse == false) // check the outpin isReverse
      QuteMsg(MSG_ERR) << "Warning : Bits reverse! Need to handle!! It doesn't finish(@CktBusCell::writeBLIF())" << endl;
//pre-process
   QuteMsg(MSG_IFO) << "totalBusNum = " << totalBusNum << endl;
   QuteMsg(MSG_IFO) << "oWidth = " << oWidth << endl;
   ...
   for (int i = 0; i < totalBusNum; ++i)
   {
      ...
      if (getInPin(i)->getOutPin() != 0) {
         ...
         if (isReverse == false) // check every inpin isReverse
            QuteMsg(MSG_ERR) << "Warning : Bits reverse! Need to handle!! It doesn't finish(@CktBusCell::writeBLIF())" << endl;
         QuteMsg(MSG_IFO) << "2. finCell type = " << aOutpin->getCell()->getCellType() << endl;
         QuteMsg(MSG_IFO) << "2. finCell type = " << aOutpin->getName() << endl;
         QuteMsg(MSG_IFO) << "2. begin ,end = " << begin << ", " << end << endl;
         solveBusWidth( aOutpin, iName, width, begin, end, isReverse, yOutpin );
         QuteMsg(MSG_IFO) << "3. begin ,end = " << begin << ", " << end << endl;
