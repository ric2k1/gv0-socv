...
CktModule::printOutpin(string& name, CktOutPin* outpin, unsigned lv, unsigned level) const
{
   ...
   */
            ...
            if (tmpBus->getWidth() == 1) {
               if (outpin->getCell() == 0)
                  ...
                  if (outpin->getCell()->getCellType() == CKT_SPLIT_CELL) {
                     ...
                     if (realBus->getWidth() != 1)
                        QuteMsg(MSG_IFO) << "[" << tmpBus->getBegin() << "]" << endl;
                  }
               }
            }
            else {
               if (tmpBus->getBegin() == tmpBus->getEnd())
                  ...
                  if (tmpBus->isInverted() == true)
                     QuteMsg(MSG_IFO) << "[" << tmpBus->getEnd() << ":" << tmpBus->getBegin() << "]"<< endl;
                  else
                     QuteMsg(MSG_IFO) << "[" << tmpBus->getBegin() << ":" << tmpBus->getEnd() << "]"<< endl;
               }
            }
            return ;
         }
   ...
   if (outpin->getName() != "") {
      for (unsigned j = 0; j < lv; j++)
         QuteMsg(MSG_IFO) << "  ";
      //QuteMsg(MSG_IFO)<< "outpin name: "<< outpin->getName()<< ", addr: "<< outpin<< ", busId: "<< outpin->getBusId()<< ",   ";
      QuteMsg(MSG_IFO) << "outpin name: " << outpin->getName();
   }
   else {
      for (unsigned j = 0; j < lv; j++)
         QuteMsg(MSG_IFO) << "  ";
      //QuteMsg(MSG_IFO)<< "outpin name: N/A, "<<", addr: "<< outpin<< ", busId: "<< outpin->getBusId()<< ",   ";
      QuteMsg(MSG_IFO) << "outpin name: N/A,  ";
   }
   
   const SynBus* tmpBus = VLDesign.getBus(outpin->getBusId());
   //QuteMsg(MSG_IFO)<< "jojo: "; tmpBus->print();
   if (tmpBus->getWidth() == 1) {
      if (outpin->getCell() == 0)
         //QuteMsg(MSG_IFO)<< ",  ";
         QuteMsg(MSG_IFO) << "[" << tmpBus->getBegin() << "]" << ",  ";
      else {
         if (outpin->getCell()->getCellType() == CKT_SPLIT_CELL) {
            const SynBus* realBus = VLDesign.getBus(outpin->getCell()->getInPin(0)->getOutPin()->getBusId());
            if (realBus->getWidth() != 1)
      ...
      if (tmpBus->getBegin() == tmpBus->getEnd())
         ...
         if (tmpBus->isInverted() == true)
            QuteMsg(MSG_IFO) << "[" << tmpBus->getEnd() << ":" << tmpBus->getBegin() << "]" << ",  ";
         else
            QuteMsg(MSG_IFO) << "[" << tmpBus->getBegin() << ":" << tmpBus->getEnd() << "]" << ",  ";
      }
   }
...
{
   //QuteMsg(MSG_IFO)<< "in cell"<< endl;
   unsigned i, j;
   string cellType = cell->getCellTypeString();
   
   if (level == 0) {
      //QuteMsg(MSG_IFO)<< "Cell type: "<< cellType<< ", "<< cell<< endl;
      QuteMsg(MSG_IFO) << "Cell type: " << cellType <<  endl;
      return ;
   }
   if (cellType == "dff") {
      //default pin
      ...
      if ((static_cast<CktConstCell*>(cell))->getBvValue() != 0) {
         if ((static_cast<CktConstCell*>(cell))->getBvValue()->value() != (unsigned)(-1))  // to check!!
            QuteMsg(MSG_IFO) << "Bv4 value: " << (static_cast<CktConstCell*>(cell))->getBvValue()->value() << endl;
         else
            QuteMsg(MSG_IFO) << "Bv4 value: " << *(static_cast<CktConstCell*>(cell))->getBvValue() << endl;
      }
   }
   else if (cellType == "pi") {
      /*for(i=0; i<lv; i++)
         QuteMsg(MSG_IFO)<< "  ";
      QuteMsg(MSG_IFO)<< "pi: "<< endl;*/
      QuteMsg(MSG_IFO) << "Cell type: " << cellType << endl;
      return;
   }
   else if (cellType == "module")
      QuteMsg(MSG_IFO) << "Cell type: " << cellType << ", module name: " 
           << (static_cast<CktModuleCell*>(cell))->getModulePtr()->getModuleName() << endl;
   else if (cellType == "equality") {
      QuteMsg(MSG_IFO) << "Cell type: " << cellType;
      if (static_cast<CktEqualityCell*>(cell)->isLogicEq())
         QuteMsg(MSG_IFO) << ", eq: logic equality" << endl;
      else
         QuteMsg(MSG_IFO) << ", eq: case equality" << endl;      
   }
   else 
      QuteMsg(MSG_IFO) << "Cell type: " << cellType << endl;
   for (i = 0; i < cell->getInPinSize(); i++) {
      for (j = 0; j < lv; j++)
         QuteMsg(MSG_IFO) << "  ";
      if ((i == 2) && (cellType == "mux"))
         QuteMsg(MSG_IFO) << "condition : " << endl;
      else
         QuteMsg(MSG_IFO) << "input " << i << " :" << endl;
      /*if (cellType == "case") {
         for (j = 0; j < lv + 2; j++)
            QuteMsg(MSG_IFO) << "  ";
         QuteMsg(MSG_IFO) << "sub-condition: ";
...
CktModule::print() const
{
   ...
   QuteMsg(MSG_IFO) << "IO List: " << endl;

   for (unsigned i = 0; i < _ioNameList.size(); i++) {
      QuteMsg(MSG_IFO) << "( " <<  _ioNameList[i] << " , "; 
      if (_ioTypeList[i] == CKT_IN)
         QuteMsg(MSG_IFO) << "CKT_IN";
      else if (_ioTypeList[i] == CKT_OUT)
         QuteMsg(MSG_IFO) << "CKT_OUT";
      else if (_ioTypeList[i] == CKT_IO)
         QuteMsg(MSG_IFO) << "CKT_IO";
      else;

      QuteMsg(MSG_IFO) << " , ";

      if ( _ioTypeList[i] == CKT_IN )
         QuteMsg(MSG_IFO) << _ioList[ CKT_IN ][ input++ ] << " )" << endl;
      else if ( _ioTypeList[i] == CKT_OUT ) {
         QuteMsg(MSG_IFO) << _ioList[ CKT_OUT ][ output ] << ", " << _ioList[CKT_OUT][output]->getOutPin() << " )" << endl;
         ++output;
      }
      else if ( _ioTypeList[i] == CKT_IO )
         QuteMsg(MSG_IFO) << _ioList[ CKT_IO ][ inout++ ] << " )" << endl;
      else
         QuteMsg(MSG_IFO) << " )" << endl;
   }
   QuteMsg(MSG_IFO) << "Lhs: reg/wire" << endl;
   for (unsigned i = 0; i < _varList.size(); i++) 
      QuteMsg(MSG_IFO) << "( " << _varList[i]->getName() << ", " << _varList[i] << " )" << endl;
}
...
CktModule::print( string outpinName, int level ) const
{
   ...
   if (i == _ioList[CKT_OUT].size()) {
      ...
      if (i == _varList.size()) 
         QuteMsg(MSG_ERR) << "Error: CktModule::print() ---> This pin name[ " << outpinName << " ] doesn't exist." << endl;
   }
}
...
CktModule::printPIO() const
{
   if (_ioList[CKT_IN].size() > 0) {
      ...
      for (unsigned i = 0; i < _ioList[CKT_IN].size(); ++i) {
         QuteMsg(MSG_IFO) << _ioList[CKT_IN][i]->getOutPin()->getName(); 
         if (i < _ioList[CKT_IN].size()-1) QuteMsg(MSG_IFO) << ", ";  
      }
   }
   if (_ioList[CKT_IO].size() > 0) {
      QuteMsg(MSG_IFO) << "PIO: ";
      for (unsigned i = 0; i < _ioList[CKT_IO].size(); ++i) {
         QuteMsg(MSG_IFO) << _ioList[CKT_IO][i]->getOutPin()->getName(); 
         if (i < _ioList[CKT_IO].size()-1) QuteMsg(MSG_IFO) << ", ";  
      }
   }
   if (_ioList[CKT_OUT].size() > 0) {
      QuteMsg(MSG_IFO) << "PO: ";
      for (unsigned i = 0; i < _ioList[CKT_OUT].size(); ++i) {
         QuteMsg(MSG_IFO) << _ioList[CKT_OUT][i]->getOutPin()->getName(); 
         if (i < _ioList[CKT_OUT].size()-1) QuteMsg(MSG_IFO) << ", ";  
      }
   }
}
...
CktModule::writeCDFG(const string pinName, const string fileName, set<unsigned>& bits) {
   ...
   if (!var) { QuteMsg(MSG_ERR) << "pinName " << pinName << " NOT exists!" << endl; return false; }
...
CktModule::getIoCell(CktIoType ioType, unsigned pos) const
{
   ...
   if (pos > _ioList[ ioType ].size()) {
      QuteMsg(MSG_ERR) << "Error: CktModule::getIoCell() ---> Index out of range." << endl;
      exit(1);
   }
...
CktModule::getIoName(unsigned index) const
{
   ...
   if (index > _ioNameList.size()) {
      QuteMsg(MSG_ERR) << "Error: CktModule::getIoName() ---> Index out of range." << endl;
      exit(1);
   }
...
CktModule::getIoType(unsigned index) const 
{
   ...
   if (index > _ioTypeList.size()) {
      QuteMsg(MSG_ERR) << "Error: CktModule::getIoType() ---> Index out of range." << endl;
      exit( 1 );
   }
...
{
   //QuteMsg(MSG_IFO) << "Total cell size = " << _cellList.size() << endl;
...
CktModule::duplicate() const
{
   ...
   for ( pos = _cellList.begin(); pos != _cellList.end(); pos++ ) {
      ...
      case CKT_LESS_CELL:
         ...
         break;
      default:
         QuteMsg(MSG_IFO) << "ERROR: Unsupport cell type( " << curCell->getCellTypeString() << "/" << curCell->getCellType() << " )" << endl;
         exit(1);
      }
      ...
      if (newCell == NULL) {
         QuteMsg(MSG_IFO) << "ERROR: Cell Duplication fault.." << endl;
         exit(1);
      }
...
CktModule::printCellList() const
{
   ...
   QuteMsg(MSG_IFO) << "cell num: " << _cellList.size() << endl;

   for (unsigned i = 0; i < _cellList.size(); ++i) {
      tmpCell = _cellList[i];
      if ( ( tmpCell->getCellType() == CKT_SPLIT_CELL )
            || ( tmpCell->getCellType() == CKT_MODULE_CELL ) ) {
         QuteMsg(MSG_IFO) << "cell: " << tmpCell->getCellTypeString() << endl;
      }
      else
         QuteMsg(MSG_IFO) << "cell: " << tmpCell->getCellTypeString() << " \t, " << tmpCell << ",  " << tmpCell->getOutPin()->getName() << endl;
   }
}
...
{
   QuteMsg(MSG_IFO) << "=====================inPin fanout Cell=================="<< endl;
   for (CktCellAry::const_iterator pos = _ioList[CKT_IN].begin(); pos != _ioList[CKT_IN].end(); pos++) {
      //QuteMsg(MSG_IFO) << "ioList[CKT_IN] = " << (*pos)->getCellType() << endl;
      QuteMsg(MSG_IFO) << "InPin Name =  " << (*pos)->getOutPin()->getName() << endl;
      for (unsigned i = 0; i < (*pos)->getOutPin()->getInPinSize(); ++i) {
         QuteMsg(MSG_IFO) << "InPin fanType " << i << " = " << (*pos)->getOutPin()->getFoutCell(i)->getCellType() << endl;
      }
   }
   QuteMsg(MSG_IFO) << "======================all cell=========================="<< endl;
   for ( CktCellAry::const_iterator pos = _cellList.begin(); pos != _cellList.end(); pos++ ) 
      QuteMsg(MSG_IFO) << "cellType = " << (*pos)->getCellType() << endl;

   QuteMsg(MSG_IFO) << "==========================end==========================="<< endl;
}
...
CktModule::moduleDuplicate(string prefix, CktModule*& fltCkt, bool isTopModule, CktCellAry& cells) const
{
   ...
   for (pos = _cellList.begin(); pos != _cellList.end(); ++pos) {
      ...
      if (cellType == CKT_MODULE_CELL) 
         ...
         //QuteMsg(MSG_ERR) << "Still not finish pioCell @ CktModule::moduleDuplicate(...)" << endl;
   }
...
CktModule::updateFltCkt() //using in "Step4: make connection between modules"
{
   ...
   QuteMsg(MSG_IFO) << "Eliminate " << count1 << " Bufs, retain " << count2 << " Bufs !!" << endl;
   forceEliminateBuf();
   adjustConst();
...
{
   QuteMsg(MSG_IFO) << "Eliminate all Bufs ... " << endl;
...
CktModule::printStatistics(bool design) const
{
   ...
   if (design) QuteMsg(MSG_IFO) << "  " << "Report Statistics : " << "Design is " << "\"" << getModuleName() << "\"" << endl;
   else QuteMsg(MSG_IFO) << "  " << "Report Statistics : " << "Ckt is " << "\"" << getModuleName() << "\"" << endl;
   QuteMsg(MSG_IFO) << "  " << "==================================================" << endl;
   for (int i = 0; i < 44; ++i) {
      if (cell_count[i] != 0) {
         QuteMsg(MSG_IFO).width(15); 
         QuteMsg(MSG_IFO) << right << CellTypeString[i] << " CELL";
         QuteMsg(MSG_IFO).width(20);
         QuteMsg(MSG_IFO) << right << cell_count[i] << endl;
      }
   }
   QuteMsg(MSG_IFO) << "  " << "==================================================" << endl;
   QuteMsg(MSG_IFO).width(20);
   QuteMsg(MSG_IFO) << right << "Total Cell";
   QuteMsg(MSG_IFO).width(20);
   QuteMsg(MSG_IFO) << right << sum << endl;
}

void
CktModule::printSummary(bool design) const
{
   if (design) QuteMsg(MSG_IFO) << "  " << "Report Summary : " << "Design is " << "\"" << getModuleName() << "\"" << endl;
   else QuteMsg(MSG_IFO) << "  " << "Report Summary : " << "Ckt is " << "\"" << getModuleName() << "\"" << endl;
   QuteMsg(MSG_IFO) << "  " << "==================================================" << endl;
   int bit_count;
   // PIs
   if (getIoSize(CKT_IN) > 0) {
      ...
      QuteMsg(MSG_IFO) << "  " << "Input Num : " << getIoSize(CKT_IN) << endl;
      QuteMsg(MSG_IFO) << "  " << "Input Bits : " << bit_count << endl;
   }
   // POs
   if (getIoSize(CKT_OUT) > 0) {
      ...
      QuteMsg(MSG_IFO) << "  " << "Output Num : " << getIoSize(CKT_OUT) << endl;
      QuteMsg(MSG_IFO) << "  " << "Output Bits : " << bit_count << endl;
   }
   // PIOs
   if (getIoSize(CKT_IO) > 0) {
      ...
      QuteMsg(MSG_IFO) << "  " << "InOut Num : " << getIoSize(CKT_IO) << endl;
      QuteMsg(MSG_IFO) << "  " << "InOut Bits : " << bit_count << endl;
   }
}
...
{
   if (design) QuteMsg(MSG_IFO) << "  " << "Report Verbose : " << "Design is " << "\"" << getModuleName() << "\"" << endl;
   else QuteMsg(MSG_IFO) << "  " << "Report Verbose : " << "Ckt is " << "\"" << getModuleName() << "\"" << endl;
   QuteMsg(MSG_IFO) << "  " << "==================================================" << endl;
   // PIs
   if (getIoSize(CKT_IN) > 0) {
      QuteMsg(MSG_IFO) << "  " << "input : ";
      for ( CktCellAry::const_iterator pos = _ioList[CKT_IN].begin(); pos != _ioList[CKT_IN].end(); pos++ ) 
         QuteMsg(MSG_IFO) << (*pos)->getOutPin()->getName() << "[" << ((*pos)->getOutWidth() - 1) << ":0]  ";
   }
   // POs
   if (getIoSize(CKT_OUT) > 0) {
      QuteMsg(MSG_IFO) << endl;
      QuteMsg(MSG_IFO) << "  " << "output : ";
...
CktModule::checkSameName(string& name)
{
   ...
   if (!(OutPin == 0)) {
      QuteMsg(MSG_IFO) << "string name " << name << " exists!" << endl;
      return false;
   }
   return true;
}
...
CktModule::creCell(const CktCellType& CellType, const int& num, int& width, const string& name)
{
   ...
   if (num == 1) {
            ...
            )
      {
         ...
         else { QuteMsg(MSG_ERR) << "Non-Logical Gates should have more than 2 inputs!" << endl; return 0; }
      ...
      //else if (CellType == CKT_MODULO_CELL) myCell = new CktModuloCell(false, myInPin, myInPin2, myOutPin);
         ...
         QuteMsg(MSG_ERR) << "BUF / INV should contain only one input!" << endl; return 0;
      }
      else { QuteMsg(MSG_ERR) << "Specified Gate should not contain 2 inputs!" << endl; return 0; }
...
CktModule::creDffCell(const int& width, const string& name, const string& clk) {
   ...
   if (!clkOutPin) { QuteMsg(MSG_ERR) << "Clock signal should be declared first!" << endl; assert (0); }
...
CktModule::SOPGate(vector<string>& inputs, string& output, set<string>& sop, bool onset)
{
   ...
   for (set<string>::const_iterator it = sop.begin(); it != sop.end(); ++it) {
      for (j = 0; j < inputs.size(); ++j) {
         if ((*it)[j] == '0') {
            ...
            // Chengyin debug
            //QuteMsg(MSG_IFO) << "INV added : " << "input = " << inputs[j] << ", output = " << name << endl;
      ...
      // Chengyin debug
      //QuteMsg(MSG_IFO) << "AND Gate : " << "\ninputs = ";
      //for(i = 0; i < gateoutpins.size(); ++i) QuteMsg(MSG_IFO) << gateoutpins[i]->getName() << "\t";
      //QuteMsg(MSG_IFO) << "\noutputs = " << name << endl;
   ...
   // Chengyin debug
   //QuteMsg(MSG_IFO) << "OR Gate : " << "\ninputs = ";
   //for(i = 0; i < andoutpins.size(); ++i) QuteMsg(MSG_IFO) << andoutpins[i]->getName() << "\t";
   //QuteMsg(MSG_IFO) << "\noutputs = " << output << endl;
...
CktModule::getParentModule() 
{
   ...
   if (curModNode == 0) {
      ...
      QuteMsg(MSG_IFO) << "Goodbye, Create Module mode!" << endl;
      return 0;
   }
   else {
      VLDesign.setCurHTNode(curModNode);
      QuteMsg(MSG_IFO) << "Back to Module : ModuleName = " << curModNode->getCktModule()->getModuleName()
...
CktModule::printMod(string& modulename, bool print) const
{
   ...
   if (print) {
      QuteMsg(MSG_IFO) << "Module name = " << myModule->getModuleName() << endl;
      QuteMsg(MSG_IFO) << "------------------------------------------" << endl;
      QuteMsg(MSG_IFO) << "Total Cell Num = " << myModule->_cellList.size() << endl;
      QuteMsg(MSG_IFO) << "The format: OutCell(CellType) --> InCell[FaninIndex](CellType)" << endl;
   }
   ...
   for (unsigned i = 0; i < myModule->_cellList.size(); ++i) {
      ...
      for (unsigned outnum = 0; outnum < mod_index ; outnum++) {
         ...
         if (myOutPin->getInPinSize() == 0) {  // NO fanout
            if (print) {
               QuteMsg(MSG_IFO) << myOutPin->getName() << "(" << myCell->getCellTypeString()
                    << ")" << " --> " << "NULL" << endl;
            }
            else if (myCell->getCellType() != CKT_PO_CELL) {
               if (!iserror) QuteMsg(MSG_IFO) << "Invalid cell list: " << endl;
               QuteMsg(MSG_IFO) << myOutPin->getName() << "(" << myCell->getCellTypeString() << ")" 
                    << " --> " << "NULL" << endl;
               iserror = true;
            }
...
CktModule::printFlatMod() const
{
   ...
   if (myModule == 0) {// modulename does not exist
      QuteMsg(MSG_IFO) << "You have to do \"Flatten\" first!" << endl;
      return 0;
   }
   CktCell* myCell;
   CktCell* tempCell;
         ...
         if (myOutPin->getInPinSize() == 0) {  // NO fanout
            ...
            for (unsigned j = 0; j < myOutPin->getInPinSize(); ++j) {
               ...
               if (tempCell->getCellType() == CKT_SPLIT_CELL) 
                  QuteMsg(MSG_IFO) << static_cast<CktSplitCell* >(tempCell)->getOutPin(temp)->getName();
               else QuteMsg(MSG_IFO) << tempCell->getOutPin()->getName();
               QuteMsg(MSG_IFO) << "(index = " << index << ") " << "(" << tempCell->getCellTypeString() << ")"; 
               QuteMsg(MSG_IFO) << endl;
            }
         }
      }
   ...
   if (_cellList.size() != 0) {
      ...
      for (CktCellAry::const_iterator pos = _cellList.begin(); pos != _cellList.end(); ++pos) {
         ...
         if (tempOutPin->getName() == gateName) {
            QuteMsg(MSG_IFO) << "  " << "Report Gate Fanins : " << "Gate is " << "\"" << gateName << "\""
                 << "  " << "Design is " << "\"" << getModuleName() << "\"" << endl;
            QuteMsg(MSG_IFO) << "  " << "Format : " << "CellType Level CellName" << endl;
            QuteMsg(MSG_IFO) << "  " << "         " << "|-> 1st Level Fanins ..." << endl;
            QuteMsg(MSG_IFO) <<  "  " << CellTypeString[(*pos)->getCellType()] << " " << (*pos)->getCellID() << " " << gateName << endl;
...
CktModule::printPOGate(string gateName, int level) const
{
   if (_cellList.size() != 0) {
      ...
      for (CktCellAry::const_iterator pos = _cellList.begin(); pos != _cellList.end(); ++pos) {
         ...
         if (tempOutPin->getName() == gateName) {
            QuteMsg(MSG_IFO) << "  " << "Report Gate Fanouts : " << "Gate is " << "\"" << gateName << "\""
                 << "  " << "Design is " << "\"" << getModuleName() << "\"" << endl;
            QuteMsg(MSG_IFO) << "  " << "Format : " << "CellType Level CellName" << endl;
            QuteMsg(MSG_IFO) << "  " << "         " << "|-> 1st Level Fanouts ..." << endl;
            QuteMsg(MSG_IFO) << "  " << CellTypeString[(*pos)->getCellType()] << " " << (*pos)->getCellID() << " " << gateName << endl;
...
CktModule::printPI(CktCell* MyCell, int level, int Maxlevel, unsigned shift) const
{
   if (level <= Maxlevel) {
      ...
      */
      for (unsigned i = 0; i < MyCell->getInPinSize(); ++i) {
         for (unsigned t = 0; t < shift; ++t) QuteMsg(MSG_IFO) << " ";
         switch (MyCell->getCellType()) {
            case CKT_DFF_CELL : 
            	QuteMsg(MSG_IFO) << "|-> " << CellTypeString[MyCell->getInPin(i)->getFinCell()->getCellType()] << " "
                               << MyCell->getInPin(i)->getFinCell()->getCellID() << " "
                               << MyCell->getInPin(i)->getOutPin()->getName() << endl;
            	break;
            /*
            case CKT_SPLIT_CELL : 
            	QuteMsg(MSG_IFO) << setw(length) << left << " --> " + static_cast<CktSplitCell* >(MyCell)->getOutPin(temp)->getName() + "(" + MyCell->getInPin(i)->getFinCell()->getCellTypeString() + ", " + int2str(level) + ")";
      ...
      */
      ...
      for (unsigned i = 0; i < FoutSize; ++i) {
         ...
         switch (tempCell->getCellType()) {
            case CKT_DFF_CELL :
                QuteMsg(MSG_IFO) << "|-> " << CellTypeString[MyCell->getFoutCell(i)->getCellType()] << " "
                               << tempCell->getCellID() << " "
                               << tempCell->getOutPin()->getName() << endl;
            	break;
            case CKT_SPLIT_CELL :
                QuteMsg(MSG_IFO) << "|-> " << CellTypeString[tempCell->getCellType()] << " "
                               << tempCell->getCellID() << " "
