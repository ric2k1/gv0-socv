...
#define PREFIX_CHAR "QuteRTL_Internal"
...
CktConstCell::writeOutput() const
{
   ...
   if (value->fullx()) {
      ...
      QuteMsg(MSG_ERR) << "Error code ### - assign non acceptable value: " << *value << endl;
...
/*
   ...
   QuteMsg(MSG_IFO) << "oName = " << oName << endl;
   QuteMsg(MSG_IFO) << "valueStr = " << getValueStr() << endl;
   QuteMsg(MSG_IFO) << "InPin size = " << getOutPin()->getInPinSize() << endl;
   QuteMsg(MSG_IFO) << "fanout type = " << getOutPin()->getInPin(0)->getCell()->getCellType() << endl;
   //QuteMsg(MSG_IFO) << "const bits = " << fanOutWidth << endl;
   QuteMsg(MSG_IFO) << "iWidth = " << iWidth << endl; */ 
   for (int i = 0; i < iWidth; ++i) {
     ...
     if ((*_value)[i] == _BV4_0) {
        ...
        QuteMsg(MSG_IFO) << "no support z(CktConstCell::writeBLIF())" << endl;
   }
...
CktSplitCell::replaceOutPin(CktOutPin* oldPin, CktOutPin* newPin)
{
   ...
   if (i == _splitOut.size()) {
      QuteMsg(MSG_ERR) << "Error : can't find the oldPin for replacing !!" << endl;
      assert (0);
   }   
}
...
{
   QuteMsg(MSG_ERR) << "Error : the cell is mulit outputs cell \"CktSplitCell\". "
...
CktMergeCell::writeBLIF() const//input pin
{
   ...
   if (isReverse == false) // check the outpin isReverse
      QuteMsg(MSG_ERR) << "Warning : Bits reverse! Need to handle!! We doesn't finish(@CktMergeCell::writeBLIF())" << endl;
   ...
   for (int i = oWidth-1; i >= 0; --i) {  // inv connection   // the same question as the SplitCell
	 ...
	 ++curMergeNum;
         ...
         if (isReverse == false) // check every inpin isReverse
            QuteMsg(MSG_ERR) << "Warning : Bits reverse! Need to handle!! It doesn't finish(@CktMergeCell::writeBLIF())" << endl;
      }
      outFile << ".names " << iName;
