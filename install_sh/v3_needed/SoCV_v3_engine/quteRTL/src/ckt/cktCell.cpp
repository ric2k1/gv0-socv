...
CktCell::getOpStr() const {
   QuteMsg(MSG_ERR) << "Error code ### - CktCellType: " << CktCellTypeStr[ _cellType ] << ", getOpStr in cktCell.h" << endl;
   return "";
}
...
{
   QuteMsg(MSG_ERR) << "Error code ### - CktCellType: " << CktCellTypeStr[_cellType] 
...
{
   QuteMsg(MSG_ERR) << "Error code ### - CktCellType: " << CktCellTypeStr[ _cellType ] << ", cellDuplicate in cktCell.h" << endl;
   return 0;
}
...
{
   QuteMsg(MSG_ERR) << "Error code ### - CktCellType: " << CktCellTypeStr[ _cellType ] << ", writeBLIF in cktCell.h" << endl;
}
...
{
   if (_outPin == 0)
      QuteMsg(MSG_IFO) << "Error1 : no outPin cellType = " << getCellType() << endl;
   if (_outPin->getInPinSize() == 0)
      QuteMsg(MSG_IFO) << "Error2 : cellType = " << getCellType() << " name = " << _outPin->getName() << " has no foutCell" << endl;
   for (unsigned i = 0; i < getInPinSize(); ++i) {
      if (getInPin(i) == 0) {
         QuteMsg(MSG_IFO) << "Error3 : cellType = " << getCellType() << " name = " << _outPin->getName() 
              << " has null inpin : number " << i << endl;
      }
      else {
         if (getInPin(i)->getOutPin() == 0) {
            QuteMsg(MSG_IFO) << "Error4 : cellType = " << getCellType() << " name = " << _outPin->getName() 
...
CktCell::DFSCktTraversal(unsigned int& index) const {
   ...
   if (--index) {
      for (unsigned i = 0; i < getInPinSize(); ++i) {
         ...
         //QuteMsg(MSG_IFO) << "Edge = " << this->getOutPin()->getName() << " --> " << cell->getOutPin()->getName() << endl;
