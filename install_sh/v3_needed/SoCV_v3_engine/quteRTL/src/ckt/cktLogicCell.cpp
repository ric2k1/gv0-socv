...
{
   //QuteMsg(MSG_IFO) << "==================BUF====================(CktBufCell :: writeBLIF() )" << endl;
   ...
   if (oWidth != iWidth)
      QuteMsg(MSG_IFO) << "Warning : unmatch # bit buf !! (iWidth, oWidth) = (" << iWidth << ", " << oWidth << ")" << endl;
...
CktBufCell::eliminate()
{
   ...
   if (aBus->getWidth() != yBus->getWidth()) { // width different => change bus Id
      QuteMsg(MSG_WAR) << "Different widths between I/O of CktBufCell !" << endl;
      QuteMsg(MSG_WAR) << "Change Input Bus to Output Bus to eliminate Buf" << endl;
...
CktXorCell::writeBwBLIF() const
{
   ...
   //QuteMsg(MSG_IFO) << "width1 = " << width1 << " width2 = " << width2 << " oWidth = " << oWidth << endl;
   //assert (width1 == width2);
...
{
   //QuteMsg(MSG_IFO) << "Warning : CktXorCell::writeRedBLIF() still not finish." << endl;
   stringstream outFile;
   int width1 =  VLDesign.getBus(getInPin(0)->getOutPin()->getBusId())->getWidth();
   string input1 = getInPin(0)->getOutPin()->getName();
   string output = getOutPin()->getName();
...
{
   // QuteMsg(MSG_IFO) << "Warning : CktXnorCell::writeRedBLIF() still not finish." << endl;
   stringstream outFile;
   int width1 =  VLDesign.getBus(getInPin(0)->getOutPin()->getBusId())->getWidth();
   string input1 = getInPin(0)->getOutPin()->getName();
   string output = getOutPin()->getName();
...
*/  
   ...
   if (aWidth != bWidth)
      QuteMsg(MSG_IFO) << "Warning : unmatch # bits comparator !! (aWidth, bWidth) = (" << aWidth << ", " << bWidth << ")" << endl;
   assert(yWidth == 1);
   if (isInvA == isInvB) {
