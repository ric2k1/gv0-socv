...
HelpCmd::usage() const
{
   QuteMsg(MSG_IFO) << "Usage: HELp [(string cmd)]" << endl;
}

void
HelpCmd::help() const
{
   QuteMsg(MSG_IFO) << setw(20) << left << "HELp: " << "Print this help message." << endl;
}
...
QuitCmd::exec(const string& option)
{
   ...
   QuteMsg(MSG_IFO) << "Are you sure to quit (Yes/No)? [No] ";
...
QuitCmd::usage() const
{
   QuteMsg(MSG_IFO) << "Usage: Quit [-Force]" << endl;
}

void
QuitCmd::help() const
{
   QuteMsg(MSG_IFO) << setw(20) << left << "Quit: " << "Quit the execution." << endl;
}
...
{
   QuteMsg(MSG_IFO) << "Usage: HIStory [(int nPrint)]" << endl;
}

void
HistoryCmd::help() const
{
   QuteMsg(MSG_IFO) << setw(20) << left << "HIStory: " << "Print command history." << endl;
}
...
{  
  QuteMsg(MSG_IFO) << "Usage: DOfile <(string filename)>" << endl;
}  
      
void
DofileCmd::help() const
{
   QuteMsg(MSG_IFO) << setw(20) << left << "DOfile: " << "Execute the commands in the dofile." << endl;
}
...
{  
   QuteMsg(MSG_IFO) << "Usage: USAGE [| -All | -Time | -Memory]" << endl;
}  
      
void
UsageCmd::help() const
{
   QuteMsg(MSG_IFO) << setw(20) << left << "USAGE: " << "Report the runtime and / or memory usage." << endl;
}
...
SLogFileCmd::exec(const string& option)
{
   ...
   if (all) { 
      QuteMsg.setAllOutFile(filename);
      QuteMsg.startAllOutFile(append);
      if (fileonly) QuteMsg.stopAllDefault();
      else QuteMsg.startAllDefault();
   }
   else {
      CmdMsgType type;
      if (cmd) type = MSG_LOG;
      ...
      QuteMsg.setOutFile(filename, type);
      QuteMsg.startOutFile(type, append);
      if (fileonly) QuteMsg.stopDefault(type);
      else if (type != MSG_LOG) QuteMsg.startDefault(type);
   }
   return CMD_EXEC_DONE;
}
...
{
   QuteMsg(MSG_IFO) << "Usage: SET LOgfile [| -All | -Cmd | -Error | -Warning | -Info | -Debug]" << endl;
   QuteMsg(MSG_IFO) << "                   <(string filename)> [| -Fileonly | -Both] [-APPend]" << endl;
}

void
SLogFileCmd::help() const
{
   QuteMsg(MSG_IFO) << setw(20) << left << "SET LOgfile: " << "Copy and/or redirect messages to assigned file." << endl;
}
