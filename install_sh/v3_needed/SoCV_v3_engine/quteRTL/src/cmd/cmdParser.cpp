...
static void mybeep()
{
   QuteMsg(MSG_IFO) << char(BEEP_CHAR);
}
...
CmdParser::printHelps(bool revealed) const
{
   ...
   for (it = _cmdLib.begin(); it != _cmdLib.end(); ++it) {
      if ((revealed) || (it->first != CMD_TYPE_REVEALED)) {
         QuteMsg(MSG_IFO) << "========== " << CmdTypeString[it->first] << " Commands : ==========" << endl;
   ...
   while (!newCmd) {
      ...
      switch (ch) {
         ...
         case NEWLINE_CHAR:    newCmd = addHistory();
                               ...
                               {
                                  QuteMsg(MSG_IFO) << char(NEWLINE_CHAR);
...
CmdParser::parseCmd(string& option)
{
   ...
   if (!e)
   {
      ...
      if ((_cmd == "ls") || (_cmd == "vi") || (_cmd == "vim") || (_cmd == "echo") || (_cmd == "cat")) 
         system(str.c_str());
      else QuteMsg(MSG_ERR) << "Illegal command!! (" << str << ")" << endl;
   }
...
CmdParser::moveBufPtr(char* const ptr)
{
   ...
   while (_readBufPtr > ptr) {
      QuteMsg(MSG_IFO) << char(BACK_SPACE_CHAR);
      --_readBufPtr;
   }

   // move right
   while (_readBufPtr < ptr) {
      QuteMsg(MSG_IFO) << *_readBufPtr;
      ++_readBufPtr;
   }
...
CmdParser::deleteLine()
{
   ...
   for (; _readBufPtr != _readBuf; --_readBufPtr)
      QuteMsg(MSG_IFO) << char(BACK_SPACE_CHAR) << " " << char(BACK_SPACE_CHAR);
...
CmdExec::errorOption(CmdOptionError err, const string& opt) const
{
   switch (err) {
      case CMD_OPT_MISSING:
         if (opt.size()) QuteMsg(MSG_ERR) << "Missing option" << " after (" << opt << ")";
         else QuteMsg(MSG_ERR) << "Missing option" << "!!" << endl;
         break;
      case CMD_OPT_EXTRA:
         QuteMsg(MSG_ERR) << "Extra option!! (" << opt << ")" << endl;
         break;
      case CMD_OPT_ILLEGAL:
         QuteMsg(MSG_ERR) << "Illegal option!! (" << opt << ")" << endl;
         break;
      case CMD_OPT_FOPEN_FAIL:
         QuteMsg(MSG_ERR) << "Error: cannot open file \"" << opt << "\"!!" << endl;
         break;
      default:
         QuteMsg(MSG_ERR) << "Unknown option error type!! (" << err << ")" << endl;
         exit(-1);
   }
