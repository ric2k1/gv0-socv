...
CmdParser1::errCmd()
{
   QuteMsg(MSG_ERR) << "Error: Illegal command \"" << _command << "\"..." << endl;
}
...
{
   if (e == SYNTAX_ERROR)
      QuteMsg(MSG_ERR) << "Error: Syntax error \"" << token
           << "\" at line " << _lineNo << endl;
   else {
      assert(e == MISSING_ARG);
      QuteMsg(MSG_ERR) << "Error: Missing argument after \"" << token
           << "\" at line " << _lineNo << endl;
   }
   return false;
}
