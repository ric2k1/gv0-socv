...
VlgModule::extractCU(map<MyString, VlpFSMSIG>& SIGS) const
{
   vector<MyString>  cus;
   MyString sigName;
   QuteMsg(MSG_IFO) << "> Possible state register of explicit CU: ";
   for (map<MyString, VlpFSMSIG>::iterator pos = SIGS.begin(); pos != SIGS.end(); pos++) {
      sigName = (pos->second).name();
      if (SIGS[sigName].isEX())  {
         QuteMsg(MSG_IFO) << sigName << " ";
         cus.push_back(sigName);
      }
   }
   QuteMsg(MSG_IFO) << endl << "> Possible state register of implitic CU: ";
   for (map<MyString, VlpFSMSIG>::iterator pos = SIGS.begin(); pos != SIGS.end(); pos++) {
      sigName = (pos->second).name();
   ...
   for (unsigned i = 0; i < cus.size(); ++i) {
      ...
      if ((tmpSIG.isEX() && tmpSIGco.isIMP()) || (tmpSIG.isIMP() && tmpSIGco.isEX())) {
         ...
         if ((sigCU != NULL && sigCoCU != NULL)) {
            ...
            QuteMsg(MSG_IFO) << "> Compose " << tmpSIG.name() << " and " << tmpSIGco.name() << endl;
...
//            sigCoCU->genPNG( "m2.png" );
   ...
   for (unsigned i = 0; i < exCU.size(); ++i) {
      if (removeMark.find(exCU[i]) == removeMark.end()) {
         ...
         QuteMsg(MSG_IFO) << "> CU Name: " << exCU[i]->name() << endl;
/*       MyString fileName;
   ...
   QuteMsg(MSG_IFO) << "Totally, " << impCU.size() << " control units extracted in this module" << endl;
   return impCU;
}
...
VlgModule::extractSIG() const 
{
   ...
   QuteMsg(MSG_IFO) << "Module " << _moduleName << ": Extract AL" << endl;
   ...
   while (curNode != NULL) {
      ...
      case NODECLASS_ASSIGN:
         ...
         newAL.reset();
         QuteMsg(MSG_IFO) << "> From ASSIGN Statement( " << curNode << " )" << endl;
            ...
            if (static_cast<const VlpOperatorNode*>(tmpNode)->getOperatorClass() == OPCLASS_CONDITIONAL) {
               ...
               do {
                  ...
                  case NODECLASS_SIGNAL:
                     ...
                     break;
                  default:
                     QuteMsg(MSG_ERR) << "VlgModule::extractAL() ---> Unsupport "
         ...
         case NODECLASS_FUNCALL:
            ...
            QuteMsg(MSG_IFO) << " > Connect Function Block: " << endl;
            nameMap.clear();
            sigMap.clear();
         ...
         break;
      default:
         QuteMsg(MSG_ERR) << "VlgModule::extractAL() ---> ERROR: Only support ASSIGN"
              << " , FUNCTION call, and ALWAYS block." << endl;
         break;
      }
      curNode = curNode->getNext();
   }
   QuteMsg(MSG_IFO) << "Module " << _moduleName << ": Extraction complete, "
...
VlgModule::genExFSM(const map<MyString, VlpFSMSIG>& SIGS) const 
{
   ...
   for (map<MyString, VlpFSMSIG>::const_iterator pos = SIGS.begin(); pos != SIGS.end(); pos++) {
      ...
      if (sig.isEX()) {
         newFSM = NULL;
         QuteMsg(MSG_IFO) << "> Explicit FSM Generation, SR: " << sig.name() << endl;
         for (unsigned i = 0; i < sig.sizeAL(); ++i) {
            ...
            if (assign.SEset().size() <= 1) {
               ...
               if (strCurState != "") {
                  ...
                  QuteMsg(MSG_IFO) << "  > New trans from state " << strCurState << " to state " << strNxtState << endl;
               }
               else { // reset state;
                  reset.push_back(strNxtState);
                  QuteMsg(MSG_IFO) << "  > Reset state " << strNxtState << endl;
               }
            }
            else {
               QuteMsg(MSG_IFO) << "  > Faild to gernerate STG, case 2" << endl;
               QuteMsg(MSG_IFO) << assign << endl;
               delete newFSM;
               newFSM = NULL;
               break;
            }
...
//======================================================================================//
         if (newFSM != NULL) {
            if (newFSM->numState() > 1) {
               ...
               QuteMsg(MSG_IFO) << "> New FSM added." << endl;
            }
            else
               delete newFSM;
...
//======================================================================================//
   ...
   QuteMsg(MSG_IFO) << "> Total FSM: " << local.size() << endl;
...
VlgModule::genExCNT(const map<MyString, VlpFSMSIG>& SIGS, const vector<MyString>& counter) const 
{
   ...
   for (unsigned i = 0; i < counter.size(); ++i) {
      ...
      QuteMsg(MSG_IFO) << "> New Explicit Counter added." << endl;
   }
   return local;
}
...
VlgModule::genImpFSM(const map< MyString, VlpFSMSIG >& SIGS) const
{
   ...
   for (map<MyString, VlpFSMSIG>::const_iterator pos = SIGS.begin(); pos != SIGS.end(); pos++) {
      sig = pos->second;
      if (sig.isIMP()) {
         QuteMsg(MSG_IFO) << "> Constructing BDD for signal: " << sig.name()
              << "[" << sig.MSB() << ":" << sig.LSB() << "]" << endl;
         buildFaild1 = false;
         buildFaild2 = false;
         reset.clear();
         if (abs(sig.MSB()-sig.LSB()) > 16) {
            QuteMsg(MSG_IFO) << "  > Too wide, skip" << endl;
            continue;
         }
         ...
         QuteMsg(MSG_IFO) << "> Construct Mapping from X --> Y (" << sig.name() << " --> " << sigCo.name() << " )" << endl;
         ...
         if (buildFaild1 == false && buildFaild2 == false) {
            ...
            local.push_back(newFSM);
            QuteMsg(MSG_IFO) << " > New Implicit FSM added" << endl;
         }
         else 
            QuteMsg(MSG_IFO) << "  > Construction faild, possibly is Counter" << endl;
         bm.reset();
...
VlgModule::genImpCNT(const map<MyString, VlpFSMSIG>& SIGS,const vector< MyString >& counter ) const 
{
   ...
   for ( i = 0; i < counter.size(); ++i ) {
      ...
      if ( hasArith ) {
         local.push_back( newCNT );
         QuteMsg(MSG_IFO) << "> New Implicit Counter added." << endl;
      }
      else {
         delete newCNT;
      }
