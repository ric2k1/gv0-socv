...
VlpDesign::creGateBLIF(vector<string>& inputs, string& output, set<string>& covers, bool onset) {
   ...
   if (inputs.size() == 0) {  // const 0/1
      ...
      //   QuteMsg(MSG_IFO) << "Cover = " << (*it) << ", Template = " << (*is) << endl;
      if (covers == gate_template) {
...
VlpDesign::parseBLIF(FILE* file) {
   ...
   while (lexLineBLIF(file)) {
      ...
      if (tokens[0] == ".model") {  // .model modName
         if (modName != "") {
            QuteMsg(MSG_ERR) << "Hierarchical design not supported in Blif-format." << endl;
            curCktModule = 0; return;
         }
         if (tokens.size() == 1) {
            QuteMsg(MSG_ERR) << "NO module name assigned after .model." << endl;
            curCktModule = 0; return;
         }
         else if (tokens.size() > 2) {
            QuteMsg(MSG_ERR) << "More than one module name assigned after .model." << endl;
            curCktModule = 0; return;
         }
         // Create top module
         modName = tokens[1];
         ...
         // Chengyin Debug
         //QuteMsg(MSG_IFO) << "Design = " << modName << endl;
      }
      else if (tokens[0] == ".inputs") {  // .inputs in1 in2 ...
         if (modName == "") {
            QuteMsg(MSG_ERR) << "Missing .model specification." << endl;
            curCktModule = 0; return;
         }
         if (input) {
            QuteMsg(MSG_ERR) << "Input ports are already assigned." << endl;
            curCktModule = 0; return;
         }
         if (tokens.size() == 1) {
            QuteMsg(MSG_ERR) << "NO input name assigned after .inputs." << endl;
            curCktModule = 0; return;
         }
         // Create PI Gates
         ...
         if (!(curCktModule->creIO(1, inputs, CKT_PI_CELL))) {
            QuteMsg(MSG_ERR) << "Create PI Gate Failed!" << endl;
            curCktModule = 0; return;
         }
         
         // Chengyin Debug
         //QuteMsg(MSG_IFO) << "Inputs = "; for (unsigned i = 0; i < inputs.size(); ++i) QuteMsg(MSG_IFO) << inputs[i] << " "; QuteMsg(MSG_IFO) << endl;
      }
         ...
         if (output) {
            QuteMsg(MSG_ERR) << "Output ports are already assigned." << endl;
            curCktModule = 0; return;
         }
         if (tokens.size() == 1) {
            QuteMsg(MSG_ERR) << "NO output name assigned after .inputs." << endl;
            curCktModule = 0; return;
         }
         // Create PO Gates
         ...
         if (!(curCktModule->creIO(1, inputs, CKT_PO_CELL))) {
            QuteMsg(MSG_ERR) << "Create PO Gate Failed!" << endl;
            curCktModule = 0; return;
         }
         
         // Chengyin Debug
         //QuteMsg(MSG_IFO) << "Outputs = "; for (unsigned i = 0; i < inputs.size(); ++i) QuteMsg(MSG_IFO) << inputs[i] << " "; QuteMsg(MSG_IFO) << endl;
      }
         ...
         if (tokens.size() < 5) {
            QuteMsg(MSG_ERR) << "Incomplete latch declaration after .latch." << endl;
            curCktModule = 0; return;
         }
         ...
         if (gateName != "") creGateBLIF(inputs, gateName, covers, onset);  // Record Gate
         if (tokens.size() == 1) {
            QuteMsg(MSG_ERR) << "NO gate name assigned after .names." << endl;
            curCktModule = 0; return;
         }
         // Create Logic Gates
         ...
         // Chengyin Debug
         //QuteMsg(MSG_IFO) << "Names = ";
         //for (unsigned i = 0; i < inputs.size(); ++i) QuteMsg(MSG_IFO) << inputs[i] << " ";
         //QuteMsg(MSG_IFO) << ", " << gateName << endl;
      }
         ...
         if (curCktModule->checkConn(modName) != 0) {
            QuteMsg(MSG_ERR) << "Check module connection failed." << endl;
            curCktModule = 0;
         }
         return;
      }
         ...
         if (gateName == "") {
            QuteMsg(MSG_ERR) << "Unknown statement \"" << tokens[0] << "\" !" << endl;
            curCktModule = 0; return;
         }
         if (inputs.size()) {
            if (tokens.size() != 2) {
               QuteMsg(MSG_ERR) << "Illegal SOP format for gate \"" << gateName << "\" !" << endl;
               curCktModule = 0; return;
            }
            //QuteMsg(MSG_IFO) << "Token0 = \"" << tokens[0] << "\" Token1 = \"" << tokens[1] << "\"" << endl;
            if (tokens[1].size() != 1) {
               QuteMsg(MSG_ERR) << "Output for gate \"" << gateName << "\" has more than one char !" << endl;
               curCktModule = 0; return;
            }
            // check onset / offset
            if ((tokens[1] != "1") && (tokens[1] != "0")) {
               QuteMsg(MSG_ERR) << "Output for gate \"" << gateName << "\" is not 0/1 !" << endl;
               curCktModule = 0; return;
            }
            if (covers.size()) {
               if (onset ^ (tokens[1] == "1")) {
                  QuteMsg(MSG_ERR) << "Gate \"" << gateName << "\" : Cannot specify both onset and offset !" << endl;
                  curCktModule = 0; return;
               }
            }
            ...
            if (tokens[0].size() != inputs.size()) {
               QuteMsg(MSG_ERR) << "Mismatch input SOP for gate \"" << gateName << "\" !" << endl;
               curCktModule = 0; return;
            }
            for (unsigned i = 0; i < tokens[0].size(); ++i) {
               if ((tokens[0][i] != '1') && (tokens[0][i] != '0') && (tokens[0][i] != '-')) {
                  QuteMsg(MSG_ERR) << "Unknown SOP specification symbol (" << tokens[0][i] << ") for gate \"" << gateName << "\" !" << endl;
                  curCktModule = 0; return;
               }
            }
            ...
            if (tokens.size() > 1) {
               QuteMsg(MSG_ERR) << "Illegal SOP format for const gate \"" << gateName << "\" !" << endl;
               curCktModule = 0; return;
            }
            if (tokens[0].size() != 1) {
               QuteMsg(MSG_ERR) << "Output for const gate \"" << gateName << "\" has more than one char !" << endl;
               curCktModule = 0; return;
            }
            // check onset / offset
            if ((tokens[0] != "1") && (tokens[0] != "0")) {
               QuteMsg(MSG_ERR) << "Output for const gate \"" << gateName << "\" is not 0/1 !" << endl;
               curCktModule = 0; return;
            }
            if (covers.size()) {
               QuteMsg(MSG_ERR) << "Const Gate \"" << gateName << "\" : Repeated specification !" << endl;
               curCktModule = 0; return;
            }
   ...
   QuteMsg(MSG_ERR) << "Are you missing .end ?!" << endl;
   curCktModule = 0;
}
...
VlpDesign::parseBTOR(FILE* file) {
   ...
   while (lexLineBTOR(file)) {
      ...
      if (!myStr2Int(tokens[0], id)) {
         QuteMsg(MSG_ERR) << "Line start with non-integer id \"" << tokens[0] << "\" !" << endl;
         curCktModule = 0; return;
      }
      // op
      if (tokens.size() == 1) {
         QuteMsg(MSG_ERR) << "Missing operator type for variable " << id << " !" << endl;
         curCktModule = 0; return;
      }
      op = tokens[1];
      // width
      if (tokens.size() == 2) {
         QuteMsg(MSG_ERR) << "Missing width specification for variable " << id << " !" << endl;
         curCktModule = 0; return;
      }
      if ((!myStr2Int(tokens[2], width)) || (!(width > 0))) {
         QuteMsg(MSG_ERR) << "Variable " << id << " without positive integer width specification \"" << tokens[2] << "\" !" << endl;
         curCktModule = 0; return;
      }
      ...
      {
         if (tokens.size() < 4) {
            QuteMsg(MSG_ERR) << "Missing operand 1 for variable " << id << " (" << op << ") !" << endl;
            curCktModule = 0; return;
         }
         ...
         if (op == "const") { creGateBTOR(width, nameFromId(id), "'b" + tokens[3]); continue; }
            ...
            QuteMsg(MSG_ERR) << "Variable " << id << " has non-integer operand \"" << tokens[3] << "\" !" << endl;
            curCktModule = 0; return;
         }
         if (op == "root") { // assertion : id root width op1
            if (width != 1) {
               QuteMsg(MSG_ERR) << "Bit-width of root should be 1 !" << endl;
               curCktModule = 0; return;
            }
            ...
            if (width != 1) {
               QuteMsg(MSG_ERR) << "Reduced variable " << id << " has width more than 1 !" << endl;
               curCktModule = 0; return;
            }
            opStr1 = creGateBTOR(op1);
            ...
            )
      {
         ...
         if (tokens.size() < 5) {
            QuteMsg(MSG_ERR) << "Missing operand 2 for variable " << id << " (" << op << ") !" << endl;
            curCktModule = 0; return;
         }
         else if (!myStr2Int(tokens[4], op2)) {
            QuteMsg(MSG_ERR) << "Variable " << id << " has non-integer operand \"" << tokens[4] << "\" !" << endl;
            curCktModule = 0; return;
         }
         ...
         if (op == "and") creGateBTOR(CKT_AND_CELL, width, nameFromId(id), opStr1, opStr2);
            ...
            QuteMsg(MSG_ERR) << "Operator \"urem\" has not been implemented !" << endl;
            curCktModule = 0; return;
         }
            ...
            if (width != 1) {
               QuteMsg(MSG_ERR) << "Comparator variable " << id << " has width more than 1 !" << endl;
               curCktModule = 0; return;
            }
            ...
            )
      {
         ...
         if (tokens.size() < 6) {
            QuteMsg(MSG_ERR) << "Missing operand 3 for variable " << id << " (" << op << ") !" << endl;
            curCktModule = 0; return;
         }
         else if (!myStr2Int(tokens[5], op3)) {
            QuteMsg(MSG_ERR) << "Variable " << id << " has non-integer operand \"" << tokens[5] << "\" !" << endl;
            curCktModule = 0; return;
         }
         ...
         QuteMsg(MSG_ERR) << "Unknown or unhandled operator \"" << op << "\" !" << endl;
         curCktModule = 0; return;
      }
   }
}
