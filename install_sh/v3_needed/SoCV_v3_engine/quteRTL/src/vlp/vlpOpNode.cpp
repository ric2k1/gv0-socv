...
{
   QuteMsg(MSG_IFO) << param << endl;
   if (l > 0) {
      if (content) {
         QuteMsg(MSG_IFO).width(i);
         QuteMsg(MSG_IFO) << " --> ";
         QuteMsg(MSG_IFO) << "ctent " << " --> "; content->whoAmI(i+11, l--);
      }
   }
}
...
{
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "int   ";
   if (l > 0)
      QuteMsg(MSG_IFO) << " : " << _number << endl;
   else
...
VlpCaseItemNode::whoAmI(int i, int l) const
{
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "CSitem";
   if (l > 0) {   
      QuteMsg(MSG_IFO) << " : ";
      if (_exp) {
         QuteMsg(MSG_IFO) << "EXP   " << " --> "; _exp->whoAmI(i+20, --l);
      }
      else 
         QuteMsg(MSG_IFO) << "EXP   " << " --> " << "default(0)" << endl;
     
      if (_inner) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
         QuteMsg(MSG_IFO) << "INNER " << " --> "; _inner->whoAmI(i+20, --l);
      }
      if (_next) {
         QuteMsg(MSG_IFO).width(i+6);
         QuteMsg(MSG_IFO) << "NEXT  " << endl;
         QuteMsg(MSG_IFO).width(i);
         QuteMsg(MSG_IFO) << " --> ";
         _next->whoAmI(i, --l);
      }
...
{ 
   QuteMsg(MSG_ERR) << "SignalNode setNext(), error syntax !!!" << endl; exit(0);
	return false; 
}
...
VlpSignalNode::whoAmI(int i, int l) const
{
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "signal";
   if (l > 0) {
      QuteMsg(MSG_IFO) << " : " << BaseModule::getName(_nameId) << endl;
      if (_msb) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
         QuteMsg(MSG_IFO) << "MSB   " << " --> "; _msb->whoAmI(i+20, --l);
      }
      if (_lsb) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
         QuteMsg(MSG_IFO) << "LSB   " << " --> "; _lsb->whoAmI(i+20, --l);
      }
   }
   else
      QuteMsg(MSG_IFO) << endl;
   ...
   if ( isBv() == false ) {
      ...
      while ( k != end ) {
         ...
         if ( node == BddNode::_error ) {
            if ( bottom + 1 > top ) {
               QuteMsg(MSG_ERR) << "The number of BDD supports exceeds "
                    << bm->supportsNum() << "." << endl;
               ret.clear();
             ...
             :VlpBaseNode(NODECLASS_PARAMETER)
{
   ...
   //QuteMsg(MSG_IFO)<< "para type: "<< (pPair->content)->getNodeClass() << "  (vlpOpNode.cpp)"<< endl;
...
{
   QuteMsg(MSG_ERR) << "ParamNode setNext(), error syntax !!!" << endl; exit(0);
   return false; 
}
...
VlpParamNode::whoAmI(int i, int l) const
{
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "param ";
   if (l > 0) {
      if (_pPair) {
         QuteMsg(MSG_IFO) << " : " << "EXP   " << " --> "; _pPair->whoAmI(i+20, --l);
      }
   }
   else
      QuteMsg(MSG_IFO) << endl;
...
{ 
   QuteMsg(MSG_ERR) << "VlpOperatorNode::whoAmI() ---> ERROR: I am operator node. " << endl;
}

void
VlpOperatorNode::writeWhoAmI(ofstream& os) const
{
   QuteMsg(MSG_ERR) << "VlpOperatorNode::writeWhoAmI() ---> ERROR: I am operator node. " << endl;
}
...
VlpOperatorNode::writeExp(set<string>& finSigs) const
{
   if (_operatorClass == OPCLASS_REPLICATION) {
      QuteMsg(MSG_ERR) << "No Support operator class \"replication\"" << endl;
      assert (0);
   }
   else if (_operatorClass == OPCLASS_CONDITIONAL) {
      QuteMsg(MSG_ERR) << "No Support operator class \"conditional\"" << endl;
      assert (0);
   }
   else {
...
VlpBitWiseOpNode::whoAmI(int i, int l) const
{
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "BWOP  " << " : " << bitwiseopclass[_bitWiseOp] << endl;
   if (l > 0) {
      if (_leftOperand) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
         QuteMsg(MSG_IFO) << "LEFT  " << " --> "; _leftOperand->whoAmI(i+20, --l);
      }
      if (_rightOperand) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
         QuteMsg(MSG_IFO) << "RIGHT " << " --> "; _rightOperand->whoAmI(i+20, --l);
      }
   }
}
...
VlpBitNotOpNode::whoAmI(int i, int l) const
{
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "BWOP  " << " : " << "(~)   "<< endl;
   if (l > 0) {
      if (_operand) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
         QuteMsg(MSG_IFO) << "OPER  " << " --> "; _operand->whoAmI(i+20, --l);
      }
   }
}
...
VlpArithOpNode::whoAmI(int i, int l) const
{
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "AROP  " << " : " << arithopclass[_arithOp] << endl;
   if (l > 0) {
      if (_leftOperand) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
...
{
   QuteMsg(MSG_ERR) << "VlpArithOpNode::writeWhoAmI() ---> ERROR: I am arithmetic node. " << endl;
}
...
VlpLogicOpNode::whoAmI(int i, int l) const
{
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "LGOP  " << " : " << logicopclass[_logicOp] << endl;
   if (l > 0) {
      if (_leftOperand) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
...
{
   QuteMsg(MSG_ERR) << "VlpLogicOpNode::writeWhoAmI() ---> ERROR: I am operator node. " << endl;
}
...
VlpReductOpNode::whoAmI(int i, int l) const
{
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "RDOP  "  << " : " << reductopclass[_reductOp] << endl;
   if (l > 0) {
      if (_operand) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
...
VlpRelateOpNode::whoAmI(int i, int l) const
{
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "RLOP  " << " : " << relateopclass[_relateOp] << endl;
   if (l > 0) {
      if (_leftOperand) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
...
VlpShiftOpNode::whoAmI(int i, int l) const
{
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "SFOP  " << " : " << shiftopclass[_shiftOp] << endl;
   if (l > 0) {
      if (_leftOperand) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
...
VlpEqualityNode::whoAmI(int i, int l) const
{
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "EQOP  "  << " : " << equalityclass[_equality] << endl;
   if (l > 0) {
      if (_leftOperand) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
...
VlpConcatenateNode::whoAmI(int i, int l) const
{
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "CONCAT";
   if (l > 0) {
      if (_content) {
         QuteMsg(MSG_IFO) << " --> "; _content->whoAmI(i+11, --l);
      }
   }
   else
      QuteMsg(MSG_IFO) << endl;
...
{ // only concatenate or replication node usage
   ...
   if (bvCount == totalCount)
      ...
      QuteMsg(MSG_WAR) << "Concatenate bv str and signal !!" << endl;
      return false;
   }
}
...
VlpReplicationNode::whoAmI(int i, int l) const
{
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "REPLIC";
   if (l > 0) {
      if (_times) {
         QuteMsg(MSG_IFO) << " : " << "TIMES " << " --> "; _times->whoAmI(i+20, --l);
      }
      if (_content) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : " << "CONT  " << " --> "; _content->whoAmI(i+20, --l);
      }
   }
   else
      QuteMsg(MSG_IFO) << endl;
...
{ // only concatenate or replication node usage
   ...
   if (bvCount == totalCount)
      ...
      QuteMsg(MSG_WAR) << "Concatenate bv str and signal!! No handle!!" << endl;
      return false;
   }
}
...
VlpConditionalNode::whoAmI(int i, int l) const
{
   ...
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "CONDOP";
   if (l > 0) {
      if (_conditional) {
         QuteMsg(MSG_IFO) << " : " << "COND  " << " --> "; _conditional->whoAmI(i+20, --l);
      }
      if (_trueChild) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
         QuteMsg(MSG_IFO) << "TRUE  " << " --> "; _trueChild->whoAmI(i+20, --l);
      }
      if (_falseChild) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
         QuteMsg(MSG_IFO) << "FALSE " << " --> "; _falseChild->whoAmI(i+20,--l);
      }
   }
   else
      QuteMsg(MSG_IFO) << endl;
