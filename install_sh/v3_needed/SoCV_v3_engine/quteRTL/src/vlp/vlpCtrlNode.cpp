...
VlpAlwaysNode::extractAL() const 
{
   ...
   int                       tab;
   QuteMsg(MSG_IFO) << "> From ALWAYS Block( " << this << " )";
   ...
   if (_exp->getNodeClass() == NODECLASS_EDGE) {
      ...
      QuteMsg(MSG_ERR) << "VlpAlwaysNode::extractAL() ---> ERROR: Faild to recognize "
   ...
   while (curNode != NULL) {
      ...
      case NODECLASS_OPERATOR: // only handle conditional operator
         if (static_cast<const VlpOperatorNode*>(curNode)->getOperatorClass() == OPCLASS_CONDITIONAL ) {
             QuteMsg(MSG_IFO) << setw(tab) << right << ">" << " Conditional node, apply SPLIT procedure" << endl;
             bufCallBack._isCase = false;
             bufCallBack._return = curNode->getNext();
             bufCallBack._cond   = size_t(NULL);
             callBack.push(bufCallBack);
      ...
      case NODECLASS_BA_OR_NBA:
         QuteMsg(MSG_IFO) << setw(tab) << right << ">" << " Assignment: " << curNode->str() << endl;
      ...
      default: // no NODECLASS_FUNCALL => need to implement if necessary 
         QuteMsg(MSG_ERR) << "VlpAlwaysNode::extractAL() ---> WARNING: Unsupported class"
              << endl;
      }
   ...
   QuteMsg(MSG_IFO) << "> End of ALWAYS block( " << this << " ), " << local.size()
        << " ALs extracted." << endl;
   return local;
}
...
VlpEdgeNode::whoAmI(int i, int l) const
{
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "edge  " << " : " << edgetype[_edgeType] << endl;
   if (l > 0) {
      if (_exp) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
...
VlpCaseNode::whoAmI(int i, int l) const
{
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "case  " << " : " << casetype[_caseType]; 
   QuteMsg(MSG_IFO) << "(isFullCase, isParallelCase) = ( " << _isFullCase << ", " 
...
VlpForNode::whoAmI(int i, int l) const
{
   ...
   if (l > 0) {
      if (_initState) {
         QuteMsg(MSG_IFO) << " : " << "INITST" << " --> "; _initState->whoAmI(i+20, --l);
      }
      if (_endCond) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
         QuteMsg(MSG_IFO) << "ENDCD " << " --> "; _endCond->whoAmI(i+20, --l); 
      }
      if (_increState) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
         QuteMsg(MSG_IFO) << "INCRST" << " --> "; _increState->whoAmI(i+20, --l);
      }
      if (_inner) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
...
VlpBAorNBA_Node::whoAmI(int i, int l) const
{
   QuteMsg(MSG_IFO) << "(" << nodeNo << ")" << endl;
   QuteMsg(MSG_IFO).width(i+6);
   QuteMsg(MSG_IFO) << "BA_NBA" << " : " << assigntype[_assignType] << endl;
   if (l > 0) {
      if (_leftSide) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
         QuteMsg(MSG_IFO) << "LSIDE " << " --> "; _leftSide->whoAmI(i+20, --l);
      }
      if (_rightSide) {
         QuteMsg(MSG_IFO).width(i+9);
         QuteMsg(MSG_IFO) << " : ";
         QuteMsg(MSG_IFO) << "RSIDE " << " --> "; _rightSide->whoAmI(i+20, --l);
      }
      if (_next) {
         QuteMsg(MSG_IFO).width(i+6);
         QuteMsg(MSG_IFO) << "NEXT  " << endl;
