...
BaseModule::printIO(bool isComplete) const
{
   if (isComplete) {
      ...
      QuteMsg(MSG_IFO) << "(";
      for (unsigned i = 0; i < _IOAry.size(); ++i) { 
         QuteMsg(MSG_IFO) << _IOAry[i]->getSigName();
         if (i != _IOAry.size()-1)
            QuteMsg(MSG_IFO) << ", ";
      }
      QuteMsg(MSG_IFO) << ") ";
...
BaseModule::dumpPortMap() const
{
   ...
   for (unsigned i = 0; i < intArr.size(); ++i)
      QuteMsg(MSG_IFO) << i << " : " << getName(intArr[i]) << endl;
}
...
BaseModule::printFlow() const
{
  ...
  for ( ; pos != NULL; pos = pos->getNext())
     QuteMsg(MSG_IFO) << pos->getNodeClass() << " -> ";
...
VlgModule::~VlgModule()
{
   ...
   //if (_POLMap != NULL)
   //   delete _POLMap;
   //QuteMsg(MSG_ERR) << "~VlgModule()" << endl;
}
...
{
   QuteMsg(MSG_IFO) << "File : " << _fileName << "    (line, col) = ( " << _line << ", " << _col << " )" << endl;
}
...
VlgModule::cstTree(VlpHierTreeNode* const parent, const ModuleMap& moduleMap) const
{
   ...
   for (unsigned i = 0; i < getInstSize(); ++i) {
      ...
      if (moduleMap.getData(instID, mn) == false) {
         QuteMsg(MSG_ERR) << "Error : The sub-module \"" << BaseModule :: getName(instID) 
...
VlgModule::NamePOL_Module(const ModuleMap& moduleMap) const
{
   if (_paramNOLMap.getSize() > 0) { //parameter overload (name mapping)
      ...
      for (unsigned i = 0; i < instNameAry.size(); ++i) {
         ...
         if (instance == NULL) {
            QuteMsg(MSG_ERR) << "Error : Instance \"" << instNameAry[i] << "\" doesn't exist in " 
                 << getFileName() << endl;
            exit(0);
         }
...
{
   QuteMsg(MSG_IFO) << "********************************************" << endl;
   QuteMsg(MSG_IFO) << "POL PTR = "<< _POLMap << endl;
   QuteMsg(MSG_IFO) << "moduleName = " << _moduleName << endl;
   /*if (_POLMap != NULL) {
      QuteMsg(MSG_IFO) << "number of OL instances = " << _POLMap->size() << endl;
      for (unsigned i = 0; i < _POLMap->size(); ++i) 
         QuteMsg(MSG_IFO) << "NO "<< i << " : number of OL parameters = " << ((*_POLMap)[i])->size() << endl;
   }*/
}
