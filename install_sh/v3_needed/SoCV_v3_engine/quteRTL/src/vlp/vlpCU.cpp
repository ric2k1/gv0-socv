...
{
   QuteMsg(MSG_ERR) << "VlpCU::writeDot() ---> ERROR: VlpCU cannot execute writeDot()"
        << endl;
}
...
{
   QuteMsg(MSG_ERR) << "VlpCU::genPNG() ---> ERROR: VlpCU cannot execute gnePNG()"
        << endl;
}
...
VlpFSM::analysis() 
{
   ...
   for (unsigned i = 0; i < _states.size(); ++i) {
      ...
      for (unsigned j = 0; j < curState->numTrans(); ++j) {
         ...
         if (curState == _reset) QuteMsg(MSG_IFO) << "> Reset state: " << nxtState->name() << endl;
      }
      if (dead && curState != _reset) {
         _deadState.push_back(curState);
         QuteMsg(MSG_IFO) << "> Dead state: " << curState->name() << endl;
      }
   }
   ...
   for (map< VlpFSMState*, bool >::iterator pos = unReach.begin(); pos != unReach.end(); pos++) {
      if (pos->second == true && pos->first->name() != "FSMReset") {
         ...
         QuteMsg(MSG_IFO) << "> Unreach state: " << pos->first->name() << endl;
      }
   }
}
...
VlpFSM::compose(VlpFSM* src) 
{
   ...
   QuteMsg(MSG_IFO) << "SRC Reset state: " << curState->numTrans() << endl;
   for ( j = 0; j < curState->numTrans(); ++j ) {
      trans = curState->getTrans(j);
      nxtState = trans.nextState();
      QuteMsg(MSG_IFO) << nxtState->name() << endl;
      if ( newFSM->getState( nxtState->name() ) != NULL ) {
         newNxtState = newFSM->getState( nxtState->name() );
         newTrans = trans;
         newTrans.setNextState( newNxtState );
         newCurState->addTrans( newTrans );
         QuteMsg(MSG_IFO) << "Add reset state: " << newNxtState->name() << endl;
      }
   }

   curState = resetState();
   newCurState = newFSM->resetState();
   QuteMsg(MSG_IFO) << "Reset State: " << curState->numTrans() << endl;
   for ( j = 0; j < curState->numTrans(); ++j ) {
      trans = curState->getTrans(j);
      nxtState = trans.nextState();
      if ( newFSM->getState( nxtState->name() ) != NULL ) {
