...
VlpFSMPSR::getPSR( unsigned int index ) const {
   if ( index < _psr.size() )
      ...
      QuteMsg(MSG_ERR) << "VlpFSMPSR::getPSR ---> WRANING: Index out of valid range."
           << endl;
   }
...
VlpFSMADS::getSig(unsigned index) const 
{
   if (index < _ads.size()) {
      ...
      QuteMsg(MSG_ERR) << "VlpFSMADS::getSig() ---> WARNING: Index out of valid range." << endl;
...
VlpFSMACS::getSig( unsigned int index ) const {
   if ( index < _acs.size() ) {
      ...
      QuteMsg(MSG_ERR) << "VlpFSMACS::getSig() ---> WARNING: Index out of valid range."
           << endl;
   }
...
VlpFSMSIG::getAL( unsigned int index ) const {
   if ( index < _al.size() ) {
      ...
      QuteMsg(MSG_ERR) << "VlpFSMSIG::getAL() ---> WARNING: Index out of valid range."
           << endl;
   }
...
/*    if ( coSIG() == name() ) {
      ...
      for (unsigned j = 0; j < resNode.size(); ++j) {
         if (resNode[j] == BddNode::_error) { // Bdd Construction faild
            QuteMsg(MSG_IFO) << "  > Constructing BDD for signal " << name() << " failed. " << endl;
            Counter.push_back(name());
            buildFaild1 = true;
            break;
         }
         else if (resNode[j] == BddNode::_one || resNode[j] == BddNode::_zero) {
            QuteMsg(MSG_IFO) << "  > Ignoring constant assignment." << endl;
            break;
         }
         else {
            sigName = "bdd_";
            ...
            QuteMsg(MSG_IFO) << "  > Constructing BDD for signal " << nodeName << " success. " << endl;
...
VlpFSMSIG::build_Y2Z(VlpFSMSIG& sigCo, vector<MyString>& Counter, vector<VlpFSMAssign>& reset, int& top, int& bottom, BddManager& bm) const
{
   ...
   if ((name() != sigCo.name()) && sigCo.isEX() == false) {
      QuteMsg(MSG_IFO) << "> Construct Mapping Y ---> Z" << endl;
      for (unsigned i = 0; i < sigCo.sizeAL() && buildFaild2 == false; ++i) {
         ...
         for (unsigned j = 0; j < resNode.size() && index >= 0; ++j) {
            if (resNode[j] == BddNode::_error) { // Bdd faild
               QuteMsg(MSG_IFO) << "  > Constructing BDD for signal " << sigCo.name() << " failed. " << endl;
...
/*    sigName  = "BDD_TR_";
      ...
      QuteMsg(MSG_IFO) << "  > Drawing BDD TR_" << name() << endl;
   ...
   if (sigCo.isEX() == false) {
      ...
      for (int i = begin; i <= end; ++i) {
         ...
         QuteMsg(MSG_IFO) << "  > Compose " << trNode.getName() << " and " << leftNode.getName() << endl;
...
/* sigName  = "BDD_";
   ...
   bm.drawBdd( nodeName, fileName );
   QuteMsg(MSG_IFO) << "  > Drawing BDD " << name() << endl;
...
{
   QuteMsg(MSG_IFO) << "  > Constructing TR for signal " << name() << "[" << MSB() << ":" << LSB() << "]" << endl;
