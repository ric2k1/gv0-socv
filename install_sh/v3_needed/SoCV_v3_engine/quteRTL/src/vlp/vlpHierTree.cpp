...
{       
   ...
   w += 5; 
   int hNum = w/5;
   QuteMsg(MSG_IFO) << "(" << hNum << ") ";
   QuteMsg(MSG_IFO) << modName << " ";
   if (isShowInst)
      QuteMsg(MSG_IFO) << _instName << " ";
   if (isShowPort)
      printPort();
...
{
   QuteMsg(MSG_IFO) << "building... " << insName << endl;
...
/*
      ...
      QuteMsg(MSG_IFO) << "creCell: " << temp->getModuleName() << " <-> " << newInsName << endl;
...
flatten_debug()
{
   IOPinConn* ioPinConn;
   int count = 0;
   QuteMsg(MSG_IFO) << "The Size of CktHierIoMap = " << hierIoMap.size() << endl;
   QuteMsg(MSG_IFO) << "========== Un-connecting ioPinConn List =============" << endl;
   ...
   QuteMsg(MSG_IFO) << "Total number of unconnecting wire = " << count << endl;
   QuteMsg(MSG_IFO) << "=====================================================" << endl;
}
...
{
   //QuteMsg(MSG_IFO) << "---------> flatten module : " << moduleName() << endl;
   //QuteMsg(MSG_IFO) << "---------> flatten inst   : " << _instName   << endl;   
...
VlpHierTree::flatten(CktModule*& fltCkt) const
{
   ...
   //QuteMsg(MSG_IFO) << "hierIoMap size = " << hierIoMap.size() << endl;
   ...
   for (CktHierIoMap::iterator pos = hierIoMap.begin(); pos != hierIoMap.end(); ++pos) {
      ...
      if (ioPinConn->isStart()) {
         //QuteMsg(MSG_IFO) << "==============" << ioPinConn->getOutPin()->getName() << "================" << endl;
...
{
   if (root != NULL) {
      //QuteMsg(MSG_IFO) << "Child size = " << root->childSize() << endl;
      for (unsigned i = 0; i < root->childSize(); ++i) {
         if (root->child(i) != 0) clear(root->child(i));
         //QuteMsg(MSG_IFO) << "End for" << endl;
      }
      //QuteMsg(MSG_IFO) << "========== Clear Node : " << root->moduleName() << " ( " << root->instName() << " ) ==========" << endl;
      delete root;
   }
}
...
VlpHierTree::printinst_mod(VlpHierTreeNode* root) const
{
   ...
   QuteMsg(MSG_IFO) << root->instName() << "(" << root->moduleName() << ")  ";
}
...
{
   _cktModules.clear();
   QuteMsg(MSG_IFO) << "  " << "CktModule Num : " << ModuleNum(_root) << endl;
   QuteMsg(MSG_IFO) << "  " << "ModuleInst Num : " << TreeNodeNum(_root) << endl;
   QuteMsg(MSG_IFO) << "  " << "Design Level : " << TreeHeight(_root) << endl;
   QuteMsg(MSG_IFO) << "  " << "Total Cell Num : " << CellNum(_root) << endl;
}
