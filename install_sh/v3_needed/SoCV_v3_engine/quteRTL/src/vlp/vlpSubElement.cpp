...
VlpInstance::~VlpInstance()
{
   //QuteMsg(MSG_IFO) << "~VlpInstance()" << endl;
}
...
VlpInstance::printInst() const
{
   int j = 0;
   QuteMsg(MSG_IFO) << "VlpInstance:" << endl;
   QuteMsg(MSG_IFO) << "> Module: " << BaseModule::getName(_moduleID)
        << ">   Name: " << _name << endl;
   if (_nArray.size() != 0) {
      QuteMsg(MSG_IFO) << "=======================================" << endl;
      QuteMsg(MSG_IFO) << "Index    Signal Name" << endl;
      QuteMsg(MSG_IFO) << "-------- ------------------------------" << endl;
      for (unsigned i = 0; i < _nArray.size(); ++i) {
         QuteMsg(MSG_IFO) << setw(8) << i << " " << BaseModule::getName(_nArray[i])
              << endl;
         j = 0;
      ...
      QuteMsg(MSG_IFO) << "=======================================" << endl;
      QuteMsg(MSG_IFO) << "Position Signal Name" << endl;
      QuteMsg(MSG_IFO) << "-------- ------------------------------" << endl;
      for (unsigned i = 0; i < _pArray.size(); ++i) {
         QuteMsg(MSG_IFO) << setw(8) << i << " "<< endl;
         j = 0;
         _pArray[i]->whoAmI(j, 100);
      }
      QuteMsg(MSG_IFO) << "=======================================" << endl;
   ...
   if (_polAry.size() > 0) {
      QuteMsg(MSG_IFO) << "> Parameter overload: " << endl;
...
{// the function only handle parameter position overload
   ...
   if (_polAry.size() != 0) {//pos mapping
      if ((_polAry.size()) > (selfModule->getParamSize())) {
         QuteMsg(MSG_ERR) << "Error : The number of overloaded parameters exceeds "
              << "the number of parameters defined in module." << endl;
         exit(0);
      }
      else {
         QuteMsg(MSG_IFO) << "=========== overload info =============" << endl;
         for (unsigned i = 0; i < (_polAry.size()); ++i) {
...
VlpPrimitive::~VlpPrimitive()
{
   //QuteMsg(MSG_IFO) << "~VlpPrimitive()" << endl;
}
...
{
   QuteMsg(MSG_IFO) << "VlpPrimitive: " << endl;
   QuteMsg(MSG_IFO) << "> Kind: " << _kind << endl;
   QuteMsg(MSG_IFO) << "> Name: " <<  BaseModule::getName( _nameID ) << endl;
   QuteMsg(MSG_IFO) << "==========================================" << endl;
   QuteMsg(MSG_IFO) << "Index    Primitive                        " << endl;
   QuteMsg(MSG_IFO) << "-------- ---------------------------------" << endl;
   for ( unsigned int i = 0; i < _pArray.size(); ++i ) {
      QuteMsg(MSG_IFO) << setw(8) << i;
...
VlpPortNode::~VlpPortNode()
{
   //delete _msb;
   //delete _lsb;
   //QuteMsg(MSG_IFO) << "~VlpPortNode()" << endl;
}
...
VlpMemAry::~VlpMemAry()
{
   //delete _arrMin;
   //delete _arrMax;
   //QuteMsg(MSG_IFO) << "~VlpMemAry()" << endl;
}
...
VlpFunction::extractAL(map<int, int>& nameMap, map<int, const VlpSignalNode*>& sigMap) const 
{
   ...
   QuteMsg(MSG_IFO) << "> From FUNCTION Block( " << this << " )";
   resolveIndex();
   ...
   for ( i = 0; i < lstSIG.size(); ++i )
      sigMark[i] = false;
   QuteMsg(MSG_IFO) << "  Name Mapping Information: " << endl;
   QuteMsg(MSG_IFO) << "  =================================================" << endl;
   QuteMsg(MSG_IFO) << "  Internal Declaration     External Function Call" << endl;
   QuteMsg(MSG_IFO) << "  ------------------------ ------------------------" << endl;
   for ( posName = nameMap.begin(); posName != nameMap.end(); posName++ ) {
      QuteMsg(MSG_IFO) << "  " << setw(24) << left << BaseModule::getName( posName->first )
...
//   curNode  = _statement;
   ...
   curNode = newNode;
   newAL.setNonSeq();
   if (newAL.isSeq()) QuteMsg(MSG_IFO) << " Sequential";
   else               QuteMsg(MSG_IFO) << " Combinational";

   QuteMsg(MSG_IFO) << " Type" << endl;
   while (curNode != NULL) {
      tab = (condStack.size() * 2) + 2;
      switch (curNode->getNodeClass()) {
      case NODECLASS_IF:
         QuteMsg(MSG_IFO) << setw(tab) << right << ">" << " Deep to IF block" << endl;
      ...
      case NODECLASS_CASE:
         ...
         QuteMsg(MSG_IFO) << setw(tab) << right << ">" << " Deep to CASE block( "
              << caseNode->str() << " )" << endl;
      ...
      case NODECLASS_CASEITEM:
         ...
         QuteMsg(MSG_IFO) << setw(tab) << right << ">" << " Deep to CASEITEM block( "
      ...
      case NODECLASS_OPERATOR:
         if (static_cast<const VlpOperatorNode*>(curNode)->getOperatorClass() == OPCLASS_CONDITIONAL ) {
             QuteMsg(MSG_IFO) << setw(tab) << right << ">" << " Conditional node, apply "
      ...
      case NODECLASS_BA_OR_NBA:
         QuteMsg(MSG_IFO) << setw(tab) << right << ">" << " Assignment" << endl;
      ...
      case NODECLASS_SIGNAL:
         ...
         break;
      default:
         QuteMsg(MSG_ERR) << "VlpAlwaysNode::extractAL() ---> WARNING: Unsupported class" << endl;
      }
      ...
      if (curNode == NULL && callBack.size() > 0) {
         do {
            ...
            if (bufCallBack._isCase == false && condStack.size() > 0) {
               condStack.pop();
               QuteMsg(MSG_IFO) << "pop cond" << endl;
            }
            QuteMsg(MSG_IFO) << "up level" << endl;
            callBack.pop();
   ...
   QuteMsg(MSG_IFO) << "> End of FUNCTION block( " << this << " ), " << local.size()
        << " ALs extracted." << endl;
/*
   for ( i = 0; i < local.size(); ++i ) {
      QuteMsg(MSG_IFO) << "AL: " << i << endl;
      QuteMsg(MSG_IFO) << local[i] << endl;
