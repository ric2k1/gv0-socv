...
{
   QuteMsg(MSG_IFO) << "Usage: REAd DEsign [-Verilog | -Blif] <[-Filelist] (string filename)>" << endl;
}

void
ReadDesignCmd::help() const 
{
   QuteMsg(MSG_IFO) << setw(20) << left << "REAd DEsign:" << "Read design or the filelist of designs." << endl;
}
...
ReadDesignCmd::valid() const
{
   if (VLDesign.isDesignRead())
      QuteMsg(MSG_WAR) << "Design exists already !!!" << endl;
   else return true;
   return false;
}
...
{
   QuteMsg(MSG_IFO) << "Usage: SYNthesis" << endl;
}

void
SynthesisCmd::help() const 
{
   QuteMsg(MSG_IFO) << setw(20) << left << "SYNthesis:" << "Perform synthesis on parsed RTL design." << endl;
}
...
SynthesisCmd::valid() const
{
   if (!VLDesign.isDesignRead())
      ...
      QuteMsg(MSG_WAR) << "Design has already been synthesized !!!" << endl;
   else return true;
   return false;
}
...
{
   QuteMsg(MSG_IFO) << "Usage: FLAtten" << endl;
}

void
FlattenCmd::help() const 
{
   QuteMsg(MSG_IFO) << setw(20) << left << "FLAtten:" << "Perform flatten on synthesized design." << endl;
}
...
ReportDesignCmd::exec(const string& option) 
{
   ...
   else QuteMsg(MSG_IFO) << "Programming Error?!" << endl;
   return CMD_EXEC_DONE;
}

void
ReportDesignCmd::usage() const {
   QuteMsg(MSG_IFO) << "Usage: REPort DEsign [-STATistics | -VERbose]" << endl;
}

void
ReportDesignCmd::help() const {
   QuteMsg(MSG_IFO) << setw(20) << left << "REPort DEsign:" << "Report hierarchical design information." << endl;
}
...
ReportCktCmd::usage() const {
   QuteMsg(MSG_IFO) << "Usage: REPort CKt [-STATistics | -VERbose]" << endl;
}

void
ReportCktCmd::help() const {
   QuteMsg(MSG_IFO) << setw(20) << left << "REPort CKt:" << "Report flattened circuit information." << endl;
}
...
{
   QuteMsg(MSG_IFO) << "Usage: REPort MOdule < |(string moduleName)> [-Level <(int nLevel)>] [-Port_list] [-Instance_list]" << endl;
}

void
ReportModuleCmd::help() const 
{
   QuteMsg(MSG_IFO) << setw(20) << left << "REPort MOdule:" << "Report module information." << endl;
}
...
{
   QuteMsg(MSG_IFO) << "Usage: REPort CDfg < | (string topModule) | (int nodeID)> [-Max_level | -Level <(int nLevel)>]" << endl;
}

void
ReportCDFGCmd::help() const 
{
   QuteMsg(MSG_IFO) << setw(20) << left << "REPort CDfg:" << "Report assigned node's CDFG information." << endl;
}
...
{
   QuteMsg(MSG_IFO) << "Usage: REPort GAte <(string gateName)> [| -FO | -FI] [-Level <(int nLevel)>]" << endl;
}

void
ReportGateCmd::help() const 
{
   QuteMsg(MSG_IFO) << setw(20) << left << "REPort GAte:" << "Report assigned gate's information." << endl;
}
...
ReportFSMCmd::exec(const string& option)
{
   ...
   // Exec Functions
   QuteMsg(MSG_ERR) << "REPort FSM has not been implemented (unusable NOW)!" << endl;
   return CMD_EXEC_DONE;
}
...
{
   QuteMsg(MSG_IFO) << "Usage: REPort FSM < | (string moduleName)>" << endl;
}

void
ReportFSMCmd::help() const
{
   QuteMsg(MSG_IFO) << setw(20) << left << "REPort FSM:" << "Report design FSM information." << endl;
}
...
ReportFSMCmd::valid() const
{
   if (!VLDesign.isDesignRead())
      ...
      QuteMsg(MSG_WAR) << "FSM has not been extracted !!!" << endl;
   else return true;
   return false;
}
...
WriteDesignCmd::usage() const {
   QuteMsg(MSG_IFO) << "Usage: WRIte DEsign [(string fileName)]" << endl;
}

void
WriteDesignCmd::help() const 
{
   QuteMsg(MSG_IFO) << setw(20) << left << "WRIte DEsign:" << "Write out hierarchical Verilog design." << endl;
}
...
WriteCktCmd::usage() const {
   QuteMsg(MSG_IFO) << "Usage: WRIte CKt [| -Verilog|-Blif] [(string fileName)]" << endl;
}

void
WriteCktCmd::help() const {
   QuteMsg(MSG_IFO) << setw(20) << left << "WRIte CKt:" << "Write out flattened Verilog or BLIF circuit." << endl;
}
...
{
   QuteMsg(MSG_IFO) << "Usage: WRIte CDfg <| (string moduleName) > <(string pinName)> " << "<(string fileName)> [| -DOT | -PNG | -PS]" << endl;
}

void
WriteCDFGCmd::help() const {
   QuteMsg(MSG_IFO) << setw(20) << left << "WRIte CDfg:" << "Write the CDFG of assigned node before synthesis." << endl;
}
...
{
   QuteMsg(MSG_IFO) << "Usage: WRIte HIErarchy < < | (string moduleName) > <(string fileName)> > [| -DOT | -PNG | -PS]" << endl;
}

void
WriteHierCmd::help() const {
   QuteMsg(MSG_IFO) << setw(20) << left << "WRIte HIErarchy:" << "Write design hierarchical relationship." << endl;
}
