...
{ 
   QuteMsg(MSG_IFO) << "Error setNext() in vlpBase.h at line " << LY->lineNo << endl; 
   return false;
}
...
{ 
   QuteMsg(MSG_IFO) << "error getNext() in vlpBase.h" << endl; 
   return NULL; 
}
...
{ 
   QuteMsg(MSG_IFO) << "Error : I am VlpBaseNode" << endl; 
}
...
{
   QuteMsg(MSG_IFO) << "Error : In VlpBase::writeExp() NodeClass = " 
        << _nodeClass << endl; 
   assert (0);
}
