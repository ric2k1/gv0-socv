...
VlpDesign::~VlpDesign()
{
   ...
   //if (_topModule != NULL)
      //delete _topModule;
   //QuteMsg(MSG_IFO) << "~VlpDesign()" << endl;
...
VlpDesign::CreateModule(const string& subName) {
   ...
   if (getCktModule(subName)) {
      QuteMsg(MSG_ERR) << "Module \"" << subName << "\" has already exist in current design !!!" << endl;
      return 0;
   }
   // Create Sub Module
...
VlpDesign::ChangeModule() {
   if (getCurHTNode() == getHier()->root()) {
      QuteMsg(MSG_WAR) << "We are already in the top module !!!" << endl;
      return 0;
   }
...
VlpDesign::getHier() const 
{
   /*
   if (_topModule == NULL) {
      QuteMsg(MSG_ERR) << "Construct hierarchy first" << endl;
      return NULL;
   }
   */
   return &_designHier;
...
VlpDesign::ParseVerilog(const char* fileName, bool isBBox)
{
   extern FILE* yyin;
   bool success = false;
   if (!(yyin = fopen(fileName, "r"))) QuteMsg(MSG_ERR) << "Verilog File \"" << fileName << "\" Doesn't Exist!!" << endl;
   else {
      // New Verilog Parser
      LY = new LY_usage();
      // Set BlackBox Toggle
      ...
      // Start File Parse
      QuteMsg(MSG_IFO) << " > Parsing Verilog File : " << getAbsPath(fileName) << " ..." << endl;
      unsigned moduleNum = getModuleNum();
      if (yyparse()) QuteMsg(MSG_ERR) << "Verilog File : " << fileName << " Parse Error!!" << endl;
      else {
...
VlpDesign::ParseFileList(const char* fileName)
{
   ...
   bool success = true;
   if (!fp) QuteMsg(MSG_ERR) << "FileList \"" << fileName << "\" Doesn't Exist!!" << endl;
   else {
      ...
      QuteMsg(MSG_IFO) << "=== Parsing FileList : " << getAbsPath(fileName) << " ..." << endl;
      while (true) {
         ...
         while (tokens) {
            if (strncmp(tokens, "//", 2) == 0) break;
               ...
               QuteMsg(MSG_WAR) << "Line " << fileLineNum << " : \"+incdir\" has not been supported yet!!" << endl;
            }
            else {
         ...
         if (library || liberty || file) {
            if (library) QuteMsg(MSG_ERR) << "Line " << fileLineNum << " : Expecting Library File after \"-LIB_VERILOG\"!!" << endl;
            else if (liberty) QuteMsg(MSG_ERR) << "Line " << fileLineNum << " : Expecting Liberty File after \"-LIB_LIBERTY\"!!" << endl;
            else if (file) QuteMsg(MSG_ERR) << "Line " << fileLineNum << " : Expecting Liberty File after \"-f\"!!" << endl;
            success = false;
         }
         if (!success) break;
      }
...
VlpDesign::ParseVerilogLib(const char* fileName)
{
   extern FILE* libin;
   bool success = false;
   if (!(libin = fopen(fileName, "r"))) QuteMsg(MSG_ERR) << "Library File \"" << fileName << "\" Doesn't Exist!!" << endl;
   else {
   // New Verilog Parser
   ...
   // Start Library Parse
   QuteMsg(MSG_IFO) << " > Parsing Verilog Library : " << getAbsPath(fileName) << " ..." << endl;
   unsigned moduleNum = getModuleNum();
   if (libparse()) QuteMsg(MSG_ERR) << "Verilog Library File : " << fileName << " Parse Error!!" << endl;
   else { 
...
VlpDesign::ParseBLIF(const char* fileName)
{
   ...
   if (!input) QuteMsg(MSG_ERR) << "BLIF File \"" << fileName << "\" Doesn't Exist!!" << endl;
   else {
      QuteMsg(MSG_IFO) << " > Parsing BLIF File : " << getAbsPath(fileName) << " ..." << endl;
...
{
   FILE* input = fopen(fileName, "r");
   if (!input) QuteMsg(MSG_ERR) << "BTOR File \"" << fileName << "\" Doesn't Exist!!" << endl;
   else {
      QuteMsg(MSG_IFO) << " > Parsing BTOR File : " << getAbsPath(fileName) << " ..." << endl;
...
VlpDesign::printCDFG( const string& mName, int level ) const {
   ...
   if ( mn == NULL ) {
      QuteMsg(MSG_ERR) << "Invalid module name \"" << mName << "\""
           << endl;
      return;
   }
   if (mn->isLibrary() == true)
      QuteMsg(MSG_ERR) << "Module \"" << mName << "\" is a library module" << endl;
   else
...
VlpDesign::repCDFG(string& modName, int level, int nID) const
{
   if (nID != -1) {
      ...
      if (ptr == NULL) {
         QuteMsg(MSG_ERR) << "Cannot find the CDFG node with ID = " << nID << "!!" << endl;
         return false;
      }
      ptr->whoAmI(0, level);
   }
      ...
      if (repMod == NULL) {
         QuteMsg(MSG_ERR) << "Cannot find module \"" << modName << "\"!!" << endl;
         return false;
      }
      if (repMod->getDataFlow() != NULL)
         repMod->getDataFlow()->whoAmI(0, level);
      else
         QuteMsg(MSG_ERR) << "No CDFG in the Module!!" << endl;
   }
   return true;
}
...
VlpDesign::repCDFG(const string& moduleName, const string& pinName) const
{
   ...
   if (!curModule) {
      QuteMsg(MSG_ERR) << "Cannot find module \"" << moduleName << "\"!!" << endl;
      return false;
   }
   ...
   if (!pinNode) {
      QuteMsg(MSG_ERR) << "Cannot find pin \"" << pinName << "\"!!" << endl;
      return false;
   }
   pinNode->whoAmI(0, 99);
   return true;
...
VlpDesign::repCktSum() const
{
   ...
   QuteMsg(MSG_IFO) << "  " << "Total Cell Num : " << _cktFlattenDesign->getCellListSize() << endl;
   return true;
}
...
VlpDesign::repFIGate(string gateName, int level) const
{
   if (!_cktFlattenDesign->printPIGate(gateName, level)) { 
      QuteMsg(MSG_ERR) << "\"" << gateName << "\" not found!." << endl;
      return false;
   }
   return true;
}
...
VlpDesign::writeHierarchy(const string moduleName, const string fileName) const
{
   ...
   if (!curNode) { QuteMsg(MSG_ERR) << "Module " << moduleName << " NOT exists in current design!" << endl; return false; }
   ofstream DOTFile;
   DOTFile.open(fileName.c_str());
   DOTFile << "digraph G {" << endl;
...
VlpDesign::repCU( const char *src ) const {
   ...
   if ( src == NULL ) {
      ...
      if ( curNode == NULL ) {
         QuteMsg(MSG_ERR) << "Module " << src << " not existed." << endl;
      }
      else if (tmpModule->isLibrary() == true)
         QuteMsg(MSG_ERR) << "Module " << src << " is LibModule." << endl;
      else {
   ...
   for ( unsigned int m = 0; m < bufModule.size(); ++m ) {
      curNode = bufModule[m];
      bufCU = curNode->CUs();
      QuteMsg(MSG_IFO) << "Module: " << curNode->getModuleName() << endl;
      QuteMsg(MSG_IFO) << "=======================================" << endl;
      QuteMsg(MSG_IFO) << "Name                 Type              " << endl;
      QuteMsg(MSG_IFO) << "-------------------- ------------------" << endl;

      for ( unsigned int i = 0; i < bufCU.size(); ++i ) {
         QuteMsg(MSG_IFO) << left << setw(20) << bufCU[i]->name().str();
         QuteMsg(MSG_IFO) << " ";

         if ( bufCU[i]->isExType() )
            QuteMsg(MSG_IFO) << " Explicit";
         else
            QuteMsg(MSG_IFO) << " Implicit";

         if ( bufCU[i]->type() == VLPCU_FSM )
            QuteMsg(MSG_IFO) << " FSM";
         else
            QuteMsg(MSG_IFO) << " Counter";
         QuteMsg(MSG_IFO) << endl;
      }

      QuteMsg(MSG_IFO) << "======================================" << endl;
      QuteMsg(MSG_IFO) << "Number of CUs: " << bufCU.size() << endl;
   }
}
...
VlpDesign::writeCU(const char *src, bool dot) const 
{
   ...
   for (unsigned i = 0; i < bufModule.size(); ++i) {
      ...
      for (unsigned i = 0; i < bufCU.size(); ++i) {
         ...
         QuteMsg(MSG_IFO) << "> Write out: " << fileName << endl;
      }
   }
}
...
{
   QuteMsg(MSG_IFO) << "Write design start..." << endl;
   if (CREMap == NULL)
   ...
   for (unsigned i = 0; i < _cktModuleAry.size(); ++i) {
      ...
      if (cktModule->isBlackBox() == false) {
         //QuteMsg(MSG_IFO) << "Writing output Module " << i+1 << " " 
   ...
   synOutFile.close();
   QuteMsg(MSG_IFO) << "Write design successfully!!" << endl;
   return true;
}
...
{
   QuteMsg(MSG_IFO) << "Write ckt start..." << endl;
   if (CREMap == NULL)
      CREMap = new CKT_usage();
   ...
   endl(synOutFile);
   synOutFile.close();
   QuteMsg(MSG_IFO) << "Write ckt successfully !!" << endl;
   return true;
}
...
{
   QuteMsg(MSG_IFO) << "Write blif start..." << endl;
   if (filename == "") cktOutFile.open("cktGate.blif");
   else cktOutFile.open(filename.c_str());
   //.module
   //QuteMsg(MSG_IFO) << "BLIF .model (VlpDesign)" << endl;
   cktOutFile << ".model BLIF_Design";
   endl(cktOutFile);
   //.input
   cktOutFile << "#=======================================inputs======================================#";
   endl(cktOutFile);
   //QuteMsg(MSG_IFO) << "BLIF .inputs (VlpDesign)" << endl;
   cktOutFile << ".inputs ";
   ...
   cktOutFile << "#======================================outputs======================================#";
   endl(cktOutFile);
   //QuteMsg(MSG_IFO) << "BLIF .outputs (VlpDesign)" << endl;
   cktOutFile << ".outputs ";
   ...
   endl(cktOutFile);
   //<command>
   //QuteMsg(MSG_IFO) << "BLIF <command> (VlpDesign)" << endl;
   _cktFlattenDesign->writeBLIF();
   //.end
   //QuteMsg(MSG_IFO) << "BLIF .end (VlpDesign)" << endl;
   cktOutFile << ".end";
   endl(cktOutFile);
   cktOutFile.close();
   QuteMsg(MSG_IFO) << "Write blif successfully !!" << endl;
   return true;
}
...
VlpDesign::hierCSTer()
{
   ...
   //QuteMsg(MSG_IFO) << "Start hierarchy construct..." << endl;
   num = 0;
   if ( top.size() != 1 ) {
   ...
   _topModule = static_cast<VlgModule*>(getModule(*pos));
	...
	QuteMsg(MSG_IFO) << "==================================================" << endl;
	QuteMsg(MSG_IFO) << "==> Total " << getModuleNum() << " Module(s)" << endl;
   QuteMsg(MSG_IFO) << "==> Top Module : " << _topModule->getModuleName() << endl;
