...
VlpConditionalNode::synthesis() const
{
   ...
   if (trueFullz || falseFullz) {
      CktOutPin* oData;
      if (trueFullz && falseFullz)
         QuteMsg(MSG_WAR) << "Both of child are high impedance." << endl;
...
VlpEqualityNode::synthesis() const
{
   ...
   if (VLDesign.getBus(oLeft->getBusId())->getWidth() !=  VLDesign.getBus(oRight->getBusId())->getWidth())
      QuteMsg(MSG_WAR) << "Bus width are not the same @ VlpEquality::synthesis()." 
...
VlpRelateOpNode::synthesis() const
{
   ...
   if (VLDesign.getBus(id1)->getWidth() != VLDesign.getBus(id2)->getWidth())
      QuteMsg(MSG_WAR) << "Bus width are not the same @ VlpRelateOpNode::synthesis()." 
                   << " LhsName = " << oLeft->getName() << " RhsName = " << oRight->getName() << endl;
...
VlpBitWiseOpNode::synthesis() const
{
   ...
   if (VLDesign.getBus(id1)->getWidth() !=  VLDesign.getBus(id2)->getWidth()) {
      QuteMsg(MSG_WAR) << "Bus width are not the same @ VlpBitWiseOpNode::synthesis()." 
                   << " LhsName = " << oLeft->getName() << " RhsName = " << oRight->getName() << endl;
   }
...
VlpConcatenateNode::getConcatMemSignal(unsigned num) const
{
   ...
   QuteMsg(MSG_ERR) << "Miss Memory signal in concatenate operator !!" << endl; assert (0);
   return 0;
} 
         ...
         if (VLDesign.getBus(oData->getBusId())->getWidth() != 0)
            ...
            if (oData->getCell() != 0) {
               if (oData->getCell()->getCellType() == CKT_CONST_CELL) {
                  QuteMsg(MSG_ERR) << "In replication operation but has no width information, value = " 
                               << static_cast<CktConstCell*>(oData->getCell())->getBvValue()->value() << endl;
               }
               else
                  QuteMsg(MSG_ERR) << "In replication operation but has no width information, name = " 
                               << oData->getName() << endl;
            }
            width += 1;
         }
...
{  // if(!exist) only gen CktOutPin, no gen SynVar
   ...
   if (_msb == 0 && _lsb == 0)
      ...
      if (busDeclared->getWidth() == 1)
         ...
         if (_msb != _lsb) {
            QuteMsg(MSG_ERR) << "Variable MSB and LSB in Signal Slicing is NOT Supported yet !!!" << endl;
            return 0;
         }
      ...
      for (unsigned i = 0; i < func->size(); ++i) {
         if (name == func->at(i)->name) {
            if (func->at(i)->ptr != 0) //only inputs have ptr !=0
               QuteMsg(MSG_ERR) << "In VlpFunCall, input cannot be lhs." << endl;
