      ...
      if (getBusId() == varOutpin->getBusId()) {
         if (this != varOutpin) {
            ...
            if (cell->getCellType() == CKT_MODULE_CELL) {
               ...
               QuteMsg(MSG_IFO) << "getCell2() = " << getCell() << endl;
               delete this;
               QuteMsg(MSG_IFO) << "getCell3() = " << getCell() << endl;
            }
...
CktOutPin::combOutPin(CktOutPin* rhsOutPin)
{
   ...
   if (lhsWidth != rhsWidth && rhsWidth > 0) {
      QuteMsg(MSG_WAR) << "combinate different width outPins" << endl;
      QuteMsg(MSG_WAR) << "rhs name = " << rhsOutPin->getName() << " lhs name = " << getName() << endl;
   }
   if (rhsWidth == 0)
      QuteMsg(MSG_WAR) << "finWidth is zero" << endl;
...
CktOutPin::connFF(CktOutPin* rhsOutPin)
{
   ...
   int rhsWidth = VLDesign.getBus(rhsOutPin->getBusId())->getWidth();
   if (lhsWidth != rhsWidth)
      QuteMsg(MSG_WAR) << "connect FF with different width outPins" << endl;
