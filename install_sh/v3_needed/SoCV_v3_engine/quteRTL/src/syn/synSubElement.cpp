...
VlpInstance::posMapping(CktModuleCell*& moduleCell) const
{
   ...
   for (int i = 0; i < vlpModule->getIoSize(); ++i) { 
      ...
      else QuteMsg(MSG_ERR) << "Unknown I/O Type for instance " << moduleCell->getInstName() << " (Type = " << port->getIoType() << ") !!!" << endl;
   }
}
...
VlpInstance::nameMapping(CktModuleCell*& moduleCell) const
{
   ...
   for (int i = 0; i < vlpModule->getIoSize(); ++i) {
      ...
      if (nArrCtr == _nArray.size())
         QuteMsg(MSG_ERR) << "Unknown I/O Name for instance " << moduleCell->getInstName() << " (name = " << ioName << ") !!!" << endl;
      if (port->getIoType() == input) {
         if (_pArray[nArrCtr] != 0) {
            ...
            QuteMsg(MSG_WAR) << "Instance with Floating PI : " << moduleCell->getInstName() << "." << ioName << endl;
         ...
         if (_pArray[nArrCtr] != 0) {
            ...
            QuteMsg(MSG_WAR) << "Instance with High Impedence PO : " << moduleCell->getInstName() << "." << ioName << endl;
...
VlpPrimitive::synthesis() const
{
   ...
   // The last element is the input, and remainings are outputs
	 ...
	 // Insert This OutPin with its BaseNode into synVarList
      ...
      QuteMsg(MSG_WAR) << "Primitive Type \"bufif\" has not supported yet !!!" << endl;
   else
      QuteMsg(MSG_ERR) << "Unknown Primitive Type (Type = " << _kind << ") !!!" << endl;
}
...
VlpPrimitive::genCellPrimitive(CktOutPin* finOut1, CktOutPin* finOut2, int kind) const
{
   ...
   if (kind == 1)      //and
      ...
      QuteMsg(MSG_ERR) << "Unable to Generate Cell : Unknown Primitive Type (Type = " << _kind << ") !!!" << endl;
...
VlpPortNode::port2Bus() const
{
   ...
   if (_msb != 0 && _lsb != 0) {
      ...
      if (msbNum == -1)
         QuteMsg(MSG_ERR) << "Port MSB Cannot be Elaborated !!!" << endl;
      else if (lsbNum == -1)
         QuteMsg(MSG_ERR) << "Port LSB Cannot be Elaborated !!!" << endl;
...
VlpMemAry::getBusId()
{
   ...
   if (_msb != 0 && _lsb != 0) {
      ...
      if (msbNum == -1)
         QuteMsg(MSG_ERR) << "Memory MSB Cannot be Elaborated !!!" << endl;
      if (lsbNum == -1)
         QuteMsg(MSG_ERR) << "Memory LSB Cannot be Elaborated !!!" << endl;
      if (msbNum > lsbNum)
         busId = VLDesign.genBusId(msbNum - lsbNum + 1, lsbNum, msbNum, true);
      else if (msbNum == lsbNum)
         busId = VLDesign.genBusId(1, lsbNum, msbNum, busDirectionDefault);
