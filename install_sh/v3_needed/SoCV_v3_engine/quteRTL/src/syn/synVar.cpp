...
{
   if (_list.size() == 0) {
      QuteMsg(MSG_ERR) << "NO load signal" << endl;
      return 0;
   }
...
{
   if (i >= _list.size()) {
      QuteMsg(MSG_ERR) << "Error index of SynVarList " << endl;
      return 0;
   }
   return _list[i];
}
   ...
   for (it = _finOutPins.begin(); it != _finOutPins.end(); ++it) {
      if (it->_busId == busId) {
         if (SYN->synCondStack.size() != 0) {
            ...
            QuteMsg(MSG_WAR) << "Mulit-driven the same bus signal!!" << " Module Name = " 
...
SynVar::connCombFin(PinAry& mergePins, bool& isMemVar) // comb part usage
{
   ...
   if (mergePins.size() == 1) {
      ...
      if (varWidth != finWidth)
         QuteMsg(MSG_WAR) << "LHS and RHS have different bus width!!(connFinPins)" << endl;
      if (varPin->getCell() != 0) {
...
SynVar::synComb(bool& isMemVar)
{
   ...
   if (finWidth != 0 && varWidth != finWidth)
      QuteMsg(MSG_WAR) << "LHS and RHS have different bus width!! VarName = " << getName() 
...
SynVar::connLhsRhs(bool isMemVar)
{
   ...
   if (_isSeq) { //data signal is at oPinRhs
      if (isBusOverLap()) {
         QuteMsg(MSG_WAR) << "BusOverLap !! finPin size = " << _finOutPins.size() << endl;
         assert(0); // not finish
         return 0;
      }
      else
...
SynVar::print() const
{
   QuteMsg(MSG_IFO) << "SynVar ==> " << "Signal Name = " << getName() << endl;
   ...
   if (bits.empty()) {
      ...
      for (set<unsigned>::const_iterator it = bits.begin(); it != bits.end(); ++it) {
         ...
         if (i == _finOutPins.size()) {
            QuteMsg(MSG_ERR) << "Bit " << (*it) << " is NOT in the bit scope of " << getName() << endl;
            return false;
         }
      }
   }
...
finPair::print() const
{
   QuteMsg(MSG_IFO) << "finPair ==> " << "_bus = [" << VLDesign.getBus(_busId)->getBegin() << " : " 
        << VLDesign.getBus(_busId)->getEnd() << "]" << "  ";
   QuteMsg(MSG_IFO) << "_finPin = " << _finPin << "  ";
   QuteMsg(MSG_IFO) << "_cond = " << _cond << endl;
   if (_cond != NULL)
      _cond->print(3);
}
...
SynMemory::synWrAddress(const VlpBaseNode* address)
{
   if (_wrAddress == NULL)
      ...
      QuteMsg(MSG_WAR) << "Manually check if the variable index of memory is consistent with others" << endl;
}
...
SynVarList::connLhsRhs()
{
   for (vector<SynVar*>::iterator it = _list.begin(); it != _list.end(); ++it) {
      //QuteMsg(MSG_IFO) << "BaseModule::connLhsRhs => handle var name = " << (*it)->getName() << endl;
      (*it)->connLhsRhs(false);
   }
...
SynCond::print(int ctr)
{
   for (int i = 0; i < ctr; i++)
      QuteMsg(MSG_IFO) << "   ";
   QuteMsg(MSG_IFO) << "SynCond ==> " << "finPin: " << _finPin << endl;
   for (int i = 0; i < ctr; i++)
      QuteMsg(MSG_IFO) << "   ";

   QuteMsg(MSG_IFO) << "cond: " << _ctrlCond << endl;
   if (_trueCond != 0) {
      for (int i = 0; i < ctr; i++)
         QuteMsg(MSG_IFO) << "   ";
      QuteMsg(MSG_IFO) << "true child: " << endl; 
      _trueCond->print(ctr + 1);
   }
   if (_falseCond != 0) {
      for (int i = 0; i < ctr; i++)
         QuteMsg(MSG_IFO) << "   ";
      QuteMsg(MSG_IFO) << "false child: " << endl; 
...
{
   for (int i = 0; i < ctr; ++i) QuteMsg(MSG_IFO) << "   ";
   QuteMsg(MSG_IFO) << "finPin: ";
   if ((refCond) && (_finPin == refCond->_finPin)) QuteMsg(MSG_IFO) << "* ";
   QuteMsg(MSG_IFO) << _finPin << endl;
   for (int i = 0; i < ctr; ++i) QuteMsg(MSG_IFO) << "   ";

   QuteMsg(MSG_IFO) << "cond: ";
   if ((refCond) && (_ctrlCond == refCond->_ctrlCond)) QuteMsg(MSG_IFO) << "* ";
   QuteMsg(MSG_IFO) << _ctrlCond << endl;
...
SynSharNodeMap::genVarDeclaredOutpin(const string& varName) const
{
   ...
   if (!exist) {
      QuteMsg(MSG_ERR) << "\"" << varName << "\" is not declared." << endl;
      assert (0);
      return 0;
   }
   else {
...
SynSharNodeMap::genVarOutPinAllFout(string& s, CktOutPin* sel)
{
   if (sel->getBusWidth() >= (sizeof(unsigned) << 3)) {
      QuteMsg(MSG_ERR) << "Index Range for Signal Slicing is Too Large (RHS = " << s
...
{
   if (i >= _list.size()) {
      QuteMsg(MSG_ERR) << "Error index of synCondNodeList " << endl;
      assert (0);
      return 0;
   }
   return _list[i];
...
SynBus::print() const
{
   QuteMsg(MSG_IFO) << "(width, begin, end, isInv ) = ( " 
        << _width << ", " << _begin << ", " << _end << ", " << _isInverted << " ) ";

   if (_isInverted)
      QuteMsg(MSG_IFO) << "[" << _end << " : " << _begin << endl;
   else
      QuteMsg(MSG_IFO) << "[" << _begin << " : " << _end << endl;
}
