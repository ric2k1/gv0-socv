...
VlpDesign::synthesis()
{
   ...
   SYN = new SYN_usage();
   QuteMsg(MSG_IFO) << "=== Start Synthesis" << " ..." << endl;
   ...
   for (unsigned i = 0; i < _vlgModuleAry.size(); ++i) {
      ...
      QuteMsg(MSG_IFO) << " > Synthesizing Module : " << curVlpModule->getModuleName() << " ..." << endl;
   ...
   QuteMsg(MSG_IFO) << "=== Cell Collection and Naming" << " ..." << endl;
   ...
   QuteMsg(MSG_IFO) << "=== Set CktModule" << " ..." << endl;
   ...
   QuteMsg(MSG_IFO) << "==> Total " << _cktModuleAry.size() << " Module(s)" << endl;
   QuteMsg(MSG_IFO) << "==> Top Module : " << topName << endl;
   return true;
}
...
VlpDesign::flatten()
{
   ...
   QuteMsg(MSG_IFO) << "==================================================" << endl;
   QuteMsg(MSG_IFO) << "===>          Module Instance Summary         <===" << endl;
   QuteMsg(MSG_IFO) << "     ==================== ===================="     << endl;
   QuteMsg(MSG_IFO) << "     ModuleName :         InstanceName : "          << endl;
   QuteMsg(MSG_IFO) << "     ==================== ===================="     << endl;

   for (unsigned i = 0; i < _cktModuleAry.size(); ++i) {
      cktModule = getCktModule(i);
      QuteMsg(MSG_IFO) << "     " << setiosflags(ios::left) << setw(19) << cktModule->getModuleName() 
   ...
   QuteMsg(MSG_IFO) << "===> Top Module : " << topName << "(" << cktModule << ")" << endl;
   QuteMsg(MSG_IFO) << "==================================================\n" << endl;
   QuteMsg(MSG_IFO) << "=== Start Flatten" << " ..." << endl;
...
VlpDesign::completeFlatten()
{
   ...
   for (unsigned i = 0; i < _cktModuleAry.size(); ++i) {
      ...
      QuteMsg(MSG_IFO) << "     " << setw(15) << left << cktModule->getModuleName() 
   ...
   QuteMsg(MSG_IFO) << "===> Top Module : " << topName << "(" << cktModule << ")" << endl;
   QuteMsg(MSG_IFO) << "==================================================\n" << endl;
   QuteMsg(MSG_IFO) << "=== Start Complete Flatten" << " ..." << endl;
   ...
   QuteMsg(MSG_IFO) << "=== Start Collect Cells" << " ..." << endl;
   _cktFlattenDesign->collectCell();
   QuteMsg(MSG_IFO) << "=== Start Buffer Elimination" << " ..." << endl;
   _designHier.updateFltCkt(_cktFlattenDesign); // recover the loss bus info of hierarchy

   QuteMsg(MSG_IFO) << "==================================================" << endl;
   QuteMsg(MSG_IFO) << "==> Total 1 Module" << endl;
   QuteMsg(MSG_IFO) << "==> Top Module : " << _cktFlattenDesign->getModuleName() << endl;
   return true;
}
...
{
   if (i >= _busArr.size()) {
      QuteMsg(MSG_ERR) << "Exceeds Bus Array Index (" << i << ", MAX = " << (_busArr.size() - 1) << ") !!!" << endl;
      return 0;
   }
   return _busArr[i];
}
