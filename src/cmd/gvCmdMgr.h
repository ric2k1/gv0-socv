...
const string GVCmdTypeString[] = {"Revealed", "Common",  "Verify",
                                  "Simulate", "Network", "Abc",
                                  "Mode"};
...
class GVCmdExec
{
    public:
        ...
        inline void      setMandCmd(const string& str) {
            _mandCmd.push_back(str);
        }
        inline const string& getMandCmd(size_t idx) const {
            return _mandCmd[idx];
        }
        inline void setOptCmd(const string& str) {
            _optCmd.push_back(str);
        }
...
struct GVCmdCompare {
        bool operator()(const GVCmdExec* a,
                        const GVCmdExec* b) const {
            return (*a < *b);
        }
};
