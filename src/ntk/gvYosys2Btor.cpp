...
struct BtorWorker2 {
        ...
        void add_nid_sig(int nid, const SigSpec& sig) {
            if (verbose) f << indent << stringf("; %d %s\n", nid, log_signal(sig));

            for (int i = 0; i < GetSize(sig); i++) bit_nid[sig[i]] = make_pair(nid, i);
        ...
        void export_cell(Cell* cell) {
            ...
            if (cell->type.in(ID($add), ID($sub), ID($mul), ID($and), ID($or), ID($xor), ID($xnor), ID($shl), ID($sshl), ID($shr), ID($sshr),
                ...
                if (btor_op == "shift" && !b_signed) btor_op = "srl";

                if (cell->type.in(ID($shl), ID($sshl), ID($shr), ID($sshr))) b_signed = false;

                if (cell->type == ID($sshr) && !a_signed) btor_op = "srl";
            ...
            if (cell->type.in(ID($logic_and), ID($logic_or), ID($logic_not))) {
                ...
                int nid = next_nid++;
                if (btor_op != "not") btorf("%d %s %d %d %d%s\n", nid, btor_op.c_str(), sid, nid_a, nid_b, getinfo(cell).c_str());
                else btorf("%d %s %d %d%s\n", nid, btor_op.c_str(), sid, nid_a, getinfo(cell).c_str());
            ...
            if (cell->type == ID($pmux)) {
                ...
                for (int i = 0; i < GetSize(sig_s); i++) {
                    ...
                    int nid2  = next_nid++;
                    if (i == GetSize(sig_s) - 1) btorf("%d ite %d %d %d %d%s\n", nid2, sid, nid_s, nid_b, nid, getinfo(cell).c_str());
                    else btorf("%d ite %d %d %d %d\n", nid2, sid, nid_s, nid_b, nid);
                    nid = nid2;
                }
            ...
            if (cell->type.in(ID($dff), ID($ff), ID($_DFF_P_), ID($_DFF_N), ID($_FF_))) {
                ...
                if (!info_filename.empty() && cell->type.in(ID($dff), ID($_DFF_P_), ID($_DFF_N_))) {
                    ...
                    if (cell->type == ID($_DFF_N_)) negedge = true;

                    if (cell->type == ID($dff) && !cell->getParam(ID::CLK_POLARITY).as_bool()) negedge = true;
                ...
                for (int i = 0; i < GetSize(sig_q); i++)
                    if (initbits.count(sig_q[i])) initval.bits.push_back(initbits.at(sig_q[i]) ? State::S1 : State::S0);
                    else initval.bits.push_back(State::Sx);

                int nid_init_val = -1;

                if (!initval.is_fully_undef()) nid_init_val = get_sig_nid(initval, -1, false, true);

                int sid = get_bv_sid(GetSize(sig_q));
                int nid = next_nid++;

                if (symbol.empty() || (!print_internal_names && symbol[0] == '$')) btorf("%d state %d\n", nid, sid);
                else btorf("%d state %d %s\n", nid, sid, log_id(symbol));

                if (nid_init_val >= 0) {
                    int nid_init = next_nid++;
                    if (verbose) btorf("; initval = %s\n", log_signal(initval));
            ...
            if (cell->is_mem_cell()) {
                ...
                for (auto& port : mem->wr_ports) {
                    if (port.clk_enable) syncwr = true;
                    else asyncwr = true;
                }

                if (asyncwr && syncwr) log_error("Memory %s.%s has mixed async/sync write ports.\n", log_id(module), log_id(mem->memid));
                ...
                if (!mem->inits.empty()) {
                    ...
                    if (constword) {
                        ...
                        for (int i = 0; i < mem->size; i++) {
                            Const thisword = initdata.extract(i * mem->width, mem->width);
                            if (thisword.is_fully_undef()) continue;
                ...
                if (mem->memid[0] == '$') btorf("%d state %d\n", nid, sid);
            ...
            if (cell->type.in(ID($adff), ID($adffe), ID($aldff), ID($aldffe), ID($dffsr), ID($dffsre)) || cell->type.str().substr(0, 5) == "$_DFF" ||
                cell->type.str().substr(0, 7) == "$_ALDFF") {
        ...
        int get_sig_nid(SigSpec sig, int to_width = -1, bool is_signed = false, bool is_init = false) {
            ...
            for (auto bit : sig)
                if (bit == State::Sx) goto has_undef_bits;
            ...
            has_undef_bits:
                ...
                for (int i = 0; i < GetSize(sig); i++)
                    if (sig[i] == State::Sx) {
                        if (first_undef < 0) first_undef = i;
                ...
                if (to_width < 0 || first_undef < to_width) {
                    ...
                    if (is_init) btorf("%d state %d\n", nid_input, sid);
                    else btorf("%d input %d\n", nid_input, sid);
            ...
            if (sig_nid.count(sig) == 0) {
                ...
                for (int i = 0; i < GetSize(sig); i++) {
                    ...
                    if (bit_nid.count(bit) == 0) {
                        if (bit.wire == nullptr) {
                            ...
                            while (i + GetSize(c) < GetSize(sig) && sig[i + GetSize(c)].wire == nullptr) c.bits.push_back(sig[i + GetSize(c)].data);
                            ...
                            for (int j = 0; j < GetSize(c); j++) nidbits.push_back(make_pair(nid, j));
                            ...
                            if (bit_cell.count(bit) == 0) {
                                ...
                                for (int j = 0; j < GetSize(s); j++) nidbits.push_back(make_pair(nid, j));
                ...
                for (int i = 0; i < GetSize(nidbits); i++) {
                    ...
                    while (i + 1 < GetSize(nidbits) && nidbits[i + 1].first == nidbits[i].first && nidbits[i + 1].second == nidbits[i].second + 1)
                        upper++, i++;
        ...
        BtorWorker2(std::ostream& f, RTLIL::Module* module, bool verbose, bool single_bad, bool cover_mode, bool print_internal_names,
                    string info_filename)
            : f(f), sigmap(module), module(module), verbose(verbose), single_bad(single_bad), cover_mode(cover_mode),
              print_internal_names(print_internal_names), info_filename(info_filename) {
            if (!info_filename.empty()) infof("name %s\n", log_id(module));
            ...
            for (auto cell : module->cells())
                if (cell->is_mem_cell()) mem_cells[cell] = mem_dict[cell->parameters.at(ID::MEMID).decode_string()];
            ...
            for (auto wire : module->wires()) {
                if (wire->attributes.count(ID::init)) {
                    ...
                    for (int i = 0; i < GetSize(wire) && i < GetSize(attrval); i++)
                        if (attrval[i] == State::S0 || attrval[i] == State::S1) initbits[sigmap(SigBit(wire, i))] = (attrval[i] == State::S1);
                }

                if (!wire->port_id || !wire->port_input) continue;
            ...
            for (auto cell : module->cells())
                for (auto& conn : cell->connections()) {
                    ...
                    for (auto bit : sigmap(conn.second)) bit_cell[bit] = cell;
                }

            for (auto wire : module->wires()) {
                if (!wire->port_id || !wire->port_output) continue;
            ...
            for (auto wire : module->wires()) {
                if (wire->port_id || wire->name[0] == '$') continue;
                ...
                if (statewires.count(wire)) continue;

                int this_nid = next_nid++;
                btorf("%d uext %d %d %d%s\n", this_nid, sid, nid, 0, getinfo(wire).c_str());
                if (info_clocks.count(nid)) info_clocks[this_nid] |= info_clocks[nid];
            ...
            while (!bad_properties.empty()) {
                ...
                if (!bad_properties.empty()) {
                    if (cursor < GetSize(todo)) bad_properties.push_back(todo[cursor++]);
            ...
            if (!info_filename.empty()) {
                ...
                if (f.fail()) log_error("Can't open file `%s' for writing: %s\n", info_filename.c_str(), strerror(errno));
                for (auto& it : info_lines) f << it;
                f.close();
            }
        }
};
...
struct BtorBackend : public Backend {
        ...
        void execute(std::ostream *&f, std::string filename, std::vector<std::string> args, RTLIL::Design *design) override
        {
                ...
                *f << stringf("; BTOR description generated by %s for module %s.\n",
                ...
                BtorWorker2(*f, topmod, verbose, single_bad, cover_mode, print_internal_names, info_filename);
...
int
CallBtorWorker(std::ostream& f, RTLIL::Module* module, bool verbose, bool single_bad, bool cover_mode, bool print_internal_names,
               string info_filename) {
    BtorWorker2(f, module, false, false, false, false, "btorinfo.log");
    return 0;
}