...
signalWidth2Type(string width){
    ...
    if (width_int <= 8)
        return "C";
    else if (width_int <= 16)
        return "S";
    else if (width_int <= 32)
        return "I";
    else if (width_int <= 64)
        return "Q";
    else
        return "W";
}
...
VerilatorAPI::genInterfaceFile(){
    ...
    if (!input_file.good())
        gvMsg(GV_MSG_ERR) << "[ERR] design_under_test.json doesn't exist" << endl;
    else{
        json rtl_info;
        ...
        for (string pi : input_vec){
            ...
            if (clk_found != string::npos)
                output_file << "\t\tclk.push_back(new SignalC" << "(\"" << pi << "\", &rootp->" << pi << ", 1));" << endl;
            else if (rst_found != end(rst_name))
                output_file << "\t\trst.push_back(new SignalC" << "(\"" << pi << "\", &rootp->" << pi << ", 1));" << endl;
            else
                output_file << "\t\tpi.push_back(new Signal" << signalWidth2Type(name2bit_map[pi]) << "(\"" << pi << "\", &rootp->" << pi << ", " << name2bit_map[pi] << "));" << endl;
        }
        output_file << "\t\t// Primary Output" << endl;
        for (string po : output_vec){
            output_file << "\t\tpo.push_back(new Signal" << signalWidth2Type(name2bit_map[po]) << "(\"" << po << "\", &rootp->" << po << ", " << name2bit_map[po] << "));" << endl;
        }
        ...
        for (string reg : output_vec){
            ...
            if (!m.empty())
                output_file << "\t\treg.push_back(new Signal" << signalWidth2Type(name2bit_map[reg]) << "(\"" << reg << "\", &rootp->" << reg << ", " << name2bit_map[reg] << "));" << endl;
        }
