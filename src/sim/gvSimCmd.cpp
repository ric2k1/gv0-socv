...
// // Reformat assertion
// vector<string>
// CRPG_reformatAssertion(vector<string> assertion_vec, string input_path,
// string output_path) {
//     gvMsg(GV_MSG_IFO) << "[INFO] Reformat the file => " << input_path <<
//     endl; ifstream input_file(input_path); ofstream output_file(output_path);
//     string   line;
...
//     smatch   m;
//     string   top_module    = yosys_design->top_module()->name.str().substr(1,
//     strlen(yosys_design->top_module()->name.c_str()) - 1);
...
//         if (!m.empty()) {
//             result         = regex_replace(line, assertion_word, "Assertion_"
//             + to_string(assertion_cnt)); result         =
//             regex_replace(result, semicolon_regex, " else set_assert_flag\("
//             + to_string(assertion_cnt) + "\)"); assertion_info = input_path +
//             ", " + to_string(assertion_cnt) + ", " + to_string(line_num) + ",
//             " + m[1].str(); assertion_vec.push_back(assertion_info);
...
//         output_file << "\t\tassertion.emplace(assertion.end(), " << assertion
//         << ")" << endl;
//     }
//     output_file << "\t}" << endl;
...
//
//     // vector<RTLIL::Module*> modules_vec =
//     yosys_design->modules().to_vector();
//     // for(int i= 0; i< modules_vec.size(); i++){
//     //     cout << modules_vec[i]->name.str() << endl;
//     // }
//     for (string file : filelist) {
//         assertion_vec = CRPG_reformatAssertion(assertion_vec, file,
//         build_path + "/" + extractFileName(file));
//     }
//
//     CRPG_genAssertionFile(build_path + "/obj_dir/assertion.hpp",
//     assertion_vec);
// }
//
// GVCmdExecStatus
...
//     gvMsg(GV_MSG_IFO) << "[INFO] Compile file -> " <<
//     gvModMgr->getInputFileName() << endl; VerilatorAPI verilator(option,
//     1000000); return GV_CMD_EXEC_DONE;
// }
...
//                       << "Build Verilator Environment for simulation use." <<
//                       endl;
// }
//----------------------------------------------------------------------
//  Verilator Test
